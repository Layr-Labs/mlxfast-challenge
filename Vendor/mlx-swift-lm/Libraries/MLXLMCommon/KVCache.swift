...
public final class LagunaSlabRotatingKVCache: RotatingKVCache, @unchecked Sendable {
    ...
    ///
    /// DONATION INVARIANT: while the slab is active, `keys`/`values` stay
    /// nil and NO plane view is retained by this object. The per-step slice
    /// update can then write the slab buffer in place (MLX donates a buffer
    /// only when nothing else shares it); a retained view would silently
    /// turn every commit into an allocate-and-copy of the whole slab —
    /// measured on the ranked runner as a -0.63% regression (`9aa844e`).
    /// Views for attention and for the `state`/`innerState` readers are
    /// built fresh on demand and die with their consumers.
    ...
    private func updateSlabInPlace(
        ...
        // Fresh views per call; nothing is retained (see donation invariant).
        if offset < maxCacheSize {
            return (
                slab[0 ..< 1, 0..., ..<offset, 0...],
                slab[1 ..< 2, 0..., ..<offset, 0...]
    ...
    public override func update(
        ...
        guard n == 1, newKeys.dim(3) == newValues.dim(3) else {
            ...
            // hit fresh caches). De-convert: materialize the planes and let
            // the stock paths take over.
            deconvertSlab()
            return super.update(keys: newKeys, values: newValues)
        }
        return updateSlabInPlace(tokens: n) { slab, at in
    ...
    private func deconvertSlab() {
        guard let slab = kvSlab else { return }
        keys = slab[0 ..< 1]
        values = slab[1 ..< 2]
        kvSlab = nil
    }
    ...
    public override func innerState() -> [MLXArray] {
        if let slab = kvSlab { return [slab] }
        return super.innerState()
    }

    public override var state: [MLXArray] {
        get {
            guard let slab = kvSlab else { return super.state }
            if offset < slab.dim(2) {
                return [
                    slab[0 ..< 1, 0..., ..<offset, 0...],
                    slab[1 ..< 2, 0..., ..<offset, 0...],
                ]
...
public final class LagunaSlabKVCache: KVCacheSimple, @unchecked Sendable {
    ...
    /// Same donation invariant as `LagunaSlabRotatingKVCache`: while active,
    /// `keys`/`values` stay nil and no plane view is retained.
    public private(set) var kvSlab: MLXArray?
    ...
    private func updateSlabAppend(
        ...
        // Fresh views per call; nothing retained (donation invariant).
        return (
            slab[0 ..< 1, 0..., ..<offset, 0...],
            slab[1 ..< 2, 0..., ..<offset, 0...]
        )
