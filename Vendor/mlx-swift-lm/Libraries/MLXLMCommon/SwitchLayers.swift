...
/// **DEFAULT `1`, and the reason is a complexity bug that only a
/// variable-length prompt can expose.**
///
/// Arm 2 looked strictly better at the ranked prefill shape: two dispatches
/// instead of three, no 1-threadgroup kernel anywhere, the same re-derive-
/// per-threadgroup pattern as `lagunaTailNormQKVGate`. That reasoning holds
/// only because 512 tokens pins `tiles` at 32.
///
/// Arm 2 dispatches `tiles` threadgroups and has EACH of them walk the whole
/// `tiles x bins` histogram to rebuild the scan, so its total work is
/// **O(tiles^2 x bins)**. Arm 1 pays one dedicated scan kernel and is
/// **O(tiles x bins)**. At 512 tokens (tiles = 32) the quadratic term is
/// invisible; at a 4000-token prompt (tiles = 250) it is ~61x more work, on
/// the prefill critical path of every sparse layer.
///
/// That is not a correctness difference — both arms are bit-exact against
/// stock `argSort` and a 34-shape sweep from 1 to 4097 rows passes on both.
/// It is a scaling difference, and the public 512-token gate cannot see it.
private let countingRouteSortMode: Int = {
    ...
    guard let value = Int(raw), (0 ... 2).contains(value) else { return 1 }
    return value
}()
