...
[[kernel]] void sdpa_vector(
  ...
  // For each key.
  //
  // unroll_count(2): the trip count is runtime (N/BN ~ 16-20 in the scored
  // decode window), so the compiler never unrolls this loop on its own and
  // each iteration's key/value loads sit behind the previous iteration's
  // serial online-softmax update. Duplicating the body once lets the
  // scheduler issue the NEXT key's loads (qk_per_thread + v_per_thread
  // vector reads) while the current iteration's exp/factor chain retires.
  // Bit-exact: partial unroll replicates iterations IN ORDER (i, then
  // i+BN) with the identical body text under -fno-fast-math; the
  // max/factor/exp accumulation chain remains strictly sequential per
  // simdgroup, so no rounding event or accumulation order changes.
