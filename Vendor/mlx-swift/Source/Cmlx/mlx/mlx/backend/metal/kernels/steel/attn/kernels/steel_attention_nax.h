...
#pragma clang loop unroll_count(4)
    ...
    for (short iq = 0; iq < TQ; iq++) {
      ...
      for (short id = 0; id < TD; id += 2) {
        ...
        // QK loop above (line ~356) -- lets the compiler interleave the next
        // V-tile load with the running mma chain instead of fully unrolling.
        // Bit-exact: the ik iterations accumulate into Otile.frag_at(iq,id) in
        // the same sequential order; unroll_count only changes instruction
        // scheduling, not accumulation order or reduction tree.
#pragma clang loop unroll_count(4)
        for (short ik = 0; ik < TK; ik++) {
          if (sg_active) {
          NAXTile<T, 1, 2> Vtile;
