...
//
// Default on after a paired local arm measured lower prefill latency; an
// explicit `0` restores the stock loads. The hoist extends 8 fragments' live
// range across the whole loop (+28 registers/thread), so ranked M5 timing is
// still the authority for the occupancy trade.
//
