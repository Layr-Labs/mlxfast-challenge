...
// promoted schedule and "64" the four-experts-per-threadgroup one before it,
// both kept as A/B controls): how many threadgroups the
...
// exactly one pipeline for the process lifetime. 256 is the one-expert-per-
// threadgroup rung the 128 schedule was staged ahead of: it takes the
// remaining prefill gain 128 left on the table, at the cost of measuring
// closer to the acceptance ceiling in the single-shot harness regime, so it
// ships as its own chunk with nothing else composed on top.
