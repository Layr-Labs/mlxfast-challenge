...
void gather_qmm_rhs_nax(
  ...
  // Exact row-major pairwise bank marker. The logical stock shape keeps the
  // public primitive contract unchanged; only this M5 expert specialization
  // consumes the compact backing. Every ordinary array retains stock
  // contiguous normalization.
  const bool pairwise_scale_marker =
      scales_.dtype() == uint8 && scales_.ndim() == 3 &&
      scales_.offset() == 0 && scales_.shape(0) == 256 &&
      scales_.shape(1) == 1024 && scales_.shape(2) == 128 &&
      scales_.strides()[0] == 1024 * 64 &&
      scales_.strides()[1] == 64 && scales_.strides()[2] == 0;
  ...
  const bool expert_pairwise_scales =
      ...
      K == 2048 && N == 1024 && scales_.shape(-1) == K / group_size;
