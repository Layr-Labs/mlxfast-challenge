...
// DARKBLOOM_EXPERT_GATHER_GROUPS (default 256; "64" restores the promoted
// four-experts-per-threadgroup schedule, "128" selects the two-experts-per-
// threadgroup interim schedule, both kept as A/B controls): how many
// threadgroups the expert-aligned gather QMM spreads the 256 experts over.
// More threadgroups means the hardware scheduler overlaps per-expert staging
// drains and MMA phases instead of serializing expert slots inside one
// threadgroup. Only the threadgroup-to-expert assignment changes: every
// output element is computed by the same tile walk with the same accumulation
// order, and the value is baked into the kernel name and template, so each
// setting compiles exactly one pipeline for the process lifetime. Measured on
// M5 Max: 64 was the promoted baseline; 128 captured roughly two-thirds of
// the 256 schedule's prefill gain while keeping the measured speedup
// comfortably mid-band; 256 (one expert per threadgroup) captures the full
// prefill gain as the staged follow-up chunk.
