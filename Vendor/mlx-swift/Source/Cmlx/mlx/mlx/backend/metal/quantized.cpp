...
//
// Second attempt: run 30580675583 repeated the pattern exactly -- every
// correctness and behavior gate passed, then both timed attempts returned
// n=4 against the floor of five. That run's log documents the cause:
// TELEM_INTERVAL_MS=100 with GPU_LOADED_UTIL=0.95, sized so the pinned
// baseline's ~1.44 s loaded window yields 9 qualifying samples. This
// candidate measures 1.965x faster than that baseline, so its loaded window
// is roughly 0.73 s and yields 4. The sampling interval was calibrated to
// the baseline's window, not the frontier's.
