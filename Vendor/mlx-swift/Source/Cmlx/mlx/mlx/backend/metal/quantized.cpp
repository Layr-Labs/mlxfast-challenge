...
// Default ON. The Swift call site supplies an explicit LHS route-to-token
// map only when this feature is enabled; the backend repeats the guard so an
// unrelated both-index gather cannot enter the Laguna expert specialization.
// Keep this default synchronized with LagunaRuntimeModel.swift: a half-enabled
// split would select incompatible graph and kernel envelopes.
...
void gather_qmm_rhs_nax(
  ...
  // Normalize the input arrays. The zero-copy arm must retain x's physical
  // token-row batch; broadcasting it would recreate the 4096-route copy.
  ...
  if (expert_aligned) {
    // The expert kernel has a stable ABI in both arms. Its disabled arm
    // receives `indices` as an unused placeholder; the enabled arm receives
...
void GatherQMM::eval_gpu(const std::vector<array>& inputs, array& out) {
  ...
  // Laguna zero-copy path. Both sorted-index flags are false when the public
  // API receives explicit indices on both sides, so identify only the exact
  // fused routed-prefill shape and require both feature guards. The generic
  // both-index dispatch remains the fallback for every other shape.
  const bool zero_copy_lhs =
