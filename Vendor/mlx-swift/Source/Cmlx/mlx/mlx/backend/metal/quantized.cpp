...
// DARKBLOOM_DIRECT_GATHER_LOADS (default ON; store-side sibling
// DARKBLOOM_DIRECT_FINAL_ROW_STORES ranked-validated at 307360f5 -- the
// encoding, Swift mirrors, kernel decode, store epilogue and JIT compile are
// all proven on the M5; "0" reverts to the stock Swift-side physical gather
// that materializes sortedX -- see `lagunaFusedSortedRoutedGateUp` in
// LagunaRuntimeModel.swift).
//
// Full-strength direct loads re-reads the gate/up A-tile from device once
// per N-tile (BN=64 over N=1024 -> 16 re-reads), cutting O(10 GB) of DRAM
// traffic per prefill pass -- plausibly enough to overshoot the paired
// prefill acceptance band on its own (see docs/benchmark-window-freeze.md).
// `DARKBLOOM_DIRECT_LOADS_LAYER_PCT` (Swift-side only,
// `lagunaDirectLoadsLayerSelected` in LagunaRuntimeModel.swift) is the
// partial-strength dial that chunks this gain across submissions, the same
// pattern as the RUNSKIP percentage dial (`DARKBLOOM_PREFILL_GATHER_RUNSKIP`,
// `darkbloom_gather_run_skip_pct` below) -- this flag alone stays a plain
// on/off switch; the layer-selection logic that turns it into a partial
// dial lives entirely in Swift and needs no counterpart here (see
// `direct_gather_loads_capable` in `GatherQMM::eval_gpu`: a layer this flag
// permits but the Swift dial excludes simply never supplies `lhsIndices`,
// so this C++ side never sees a difference between "dial excluded this
// layer" and "loads was never eligible here to begin with").
//
...
void gather_qmm_rhs_nax(
  ...
  //   bit 2 (4): indices-are-encoded. The Swift callers encode the shared
  //              per-layer index array whenever EITHER lever's eligibility
  //              holds (see `lagunaFusedSortedRoutedGateUp`), so the
  //              kernel's run-boundary decode must fire on every
  //              expert-aligned dispatch whenever either global flag is
  //              on -- NOT only when this specific dispatch's own leg bit
  //              is set. Without this bit, loads-on/stores-off would hand
  //              the down leg encoded indices with a zero scalar and
  //              corrupt its run search (audit finding, hardened here).
  const bool indices_may_be_encoded =
      darkbloom_direct_final_row_stores_enabled() ||
      darkbloom_direct_gather_loads_enabled();
  const int trailing_scalar_arg = expert_aligned
      ? ((direct_final_store ? 1 : 0) | (direct_gather_loads ? 2 : 0) |
         (indices_may_be_encoded ? 4 : 0))
      : run_skip_pct;
