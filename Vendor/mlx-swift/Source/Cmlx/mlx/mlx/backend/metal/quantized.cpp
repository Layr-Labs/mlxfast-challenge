...
// DARKBLOOM_EXPERT_STAGE_WIDEST (default ON; "0" = scalar staging A/B
// control): wide 16B threadgroup stores in the expert gather QMM loader
// (load_unsafe_wide<true, false>). Store-side only: Ws is 16B aligned by
// construction (NAXWsChunk16) and every destination offset is a multiple of
// 16 at the shipped BN=64/BK=64 geometry, so no device access widens.
// Identical values at identical addresses; only store width changes. Baked
// into the kernel name, so one pipeline per setting per process.
...
// DARKBLOOM_EXPERT_STAGE_WIDELD (default ON; "0" = narrow loads A/B
// control): 8B device loads in the same expert loader. Host certifies the
// source alignment; values and addresses are unchanged.
