...
void gather_qmm_rhs_nax(
  ...
  // Both expert-aligned legs share one encoded index array. Even when direct
  // final stores are off, the down leg must decode expert ids when the gate/up
  // leg uses direct gathers. Bit 2 carries that cross-leg contract.
