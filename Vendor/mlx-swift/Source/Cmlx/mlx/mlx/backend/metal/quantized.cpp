...
int darkbloom_stage_bm128_variant() {
  static const int v = [] {
    ...
    if (s.empty()) {
      // Default 5 (2026-07-31/08-01, GatherX/Main): BM64/WM4/WN1 on the
      // wn1-admitting expert path: -4.2% gate/up, -3.0% down kernel-level
      // (bit-identical), -1.34% prefill wall (5/5 like-regime pairs, both
      // orders, Welch t~6), decode unchanged by construction. Its first
      // ranked appearance (f39c080a) drew -1.76% against a same-hour +1.44%
      // promotion of the identical base stack; this re-prices the arm
      // isolated on the current frontier. DARKBLOOM_STAGE_BM128=4 restores.
      return 5;
    }
    if (s == "1") {
      return 1;
