...
static inline void fp4nv_decode8(uint32_t c, float scale, thread T* out) {
  // Bit-exact nibble-split extract (same half patterns as stock shuffle).
  ...
  const float2 v0 = float2(as_type<half2>(p0)) * scale;
  const float2 v1 = float2(as_type<half2>(p1)) * scale;
  const float2 v2 = float2(as_type<half2>(p2)) * scale;
  const float2 v3 = float2(as_type<half2>(p3)) * scale;
  out[0] = T(v0.x);
  out[1] = T(v1.x);
  out[2] = T(v2.x);
  out[3] = T(v3.x);
