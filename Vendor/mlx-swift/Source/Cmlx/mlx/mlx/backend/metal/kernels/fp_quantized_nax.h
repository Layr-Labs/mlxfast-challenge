...
[[kernel]] void fp_gather_qmm_rhs_expert_nax(
  ...
  //   bit 2: the shared per-layer index array is ENCODED (set whenever
  //          either lever's global flag is on and this dispatch is
  //          expert-aligned) -- consulted only via the run_skip_pct != 0
  //          check below, so a decode-only dispatch (e.g. the down leg
  //          under loads-on/stores-off) still searches decoded ids.
