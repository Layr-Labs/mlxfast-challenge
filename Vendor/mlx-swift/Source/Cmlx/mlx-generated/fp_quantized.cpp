...
// scales4 = 1 selects the packed-scale-plane arm: the scale bytes are
// PERMUTED at init from [row][group] row-major into 4-row bundles
// ([row/4][group][row%4]), so the four scattered 1-byte reads per k-block
// (rows 128 B apart, 32 useful bytes per touched cache line) become ONE
// aligned uchar4 whose 32-lane footprint is exactly one fully-consumed
// 128-byte cache line. Same bytes consumed for the same (row, group) in
// the same order — a pure relayout, bit-exact by construction. Only valid
// for group_size 16 (scale_step 1) with out_row a multiple of 4, which the
// host guard pins.
...
METAL_FUNC void fp_qmv_fast_impl(
  ...
  if (scales4) {
    // Packed plane: byte offset (out_row/4)*(G*4) + g*4 == out_row*G +
    // g*4 since out_row % 4 == 0; lane's first group g == simd_lid.
