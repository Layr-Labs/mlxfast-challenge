...
template <
    ...
    bool kAlignedK,
    typename AccumType = float>
void gemm_loop_dense_prefill_swiglu(
    thread NAXTile<AccumType, SM / 16, SN / 16>& Dtile_gate,
    thread NAXTile<AccumType, SM / 16, SN / 16>& Dtile_up,
    const device T* A,
  ...
  const int gemm_k_iterations_ = gemm_k_iterations_aligned;
  STEEL_PRAGMA_NO_UNROLL
  for (int kk0 = 0; kk0 < gemm_k_iterations_; kk0++) {
    threadgroup_barrier(mem_flags::mem_none);
    ...
    for (int kk1 = 0; kk1 < BK; kk1 += SK) {
      NAXTile<T, RA, CA> Atile;
      NAXTile<T, RB, CB> Btile_gate;
      NAXTile<T, RB, CB> Btile_up;
      const int k = kk1;
      ...
      if constexpr (kAlignedN) {
        Btile_gate.load(B_gate + B_offset, ldb);
        Btile_up.load(B_up + B_offset, ldb);
      } else {
        const short rmax = transpose_b ? sgp_sn : SK;
        const short cmax = transpose_b ? SK : sgp_sn;
        Btile_gate.load_safe(
            B_gate + B_offset, ldb, short2(cmax, rmax));
        Btile_up.load_safe(B_up + B_offset, ldb, short2(cmax, rmax));
      }

      tile_matmad_nax(Dtile_gate, Atile, metal::bool_constant<transpose_a>{},
          Btile_gate, metal::bool_constant<transpose_b>{});
      tile_matmad_nax(Dtile_up, Atile, metal::bool_constant<transpose_a>{},
          Btile_up, metal::bool_constant<transpose_b>{});

      (void)compiler_barrier;
    }

    A += transpose_a ? (BK * lda) : BK;
    B_gate += transpose_b ? BK : (BK * ldb);
    B_up += transpose_b ? BK : (BK * ldb);
  }
  ...
  if constexpr (!kAlignedK) {
    ...
    for (int kk1 = 0; kk1 < rem_bk; kk1 += SK) {
      ...
      Atile.load_safe(A + A_offset, lda, Aklims);
      Btile_gate.load_safe(B_gate + B_offset, ldb, Bklims);
      Btile_up.load_safe(B_up + B_offset, ldb, Bklims);
