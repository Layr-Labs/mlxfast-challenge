...
[[kernel]] void fp_gather_qmm_rhs_expert_nax(
  ...
  for (int expert_slot = 0; expert_slot < experts / expert_groups;
    ...
    for (int chunk_start = run_start; chunk_start < run_end;
      ...
      if (fuse_swiglu) {
#ifdef DARKBLOOM_SWIGLU_REGLOCAL
        // Register-local swiglu: computed straight from Dtile fragments,
        // no gate_up_stage round-trip.
...
#pragma clang fp contract(off)
        ...
        // Dead under DARKBLOOM_SWIGLU_REGLOCAL: both epilogues then read
        // only registers, and the next chunk's k-loop opens its own WAR
        // barrier, so this one guards nothing.
