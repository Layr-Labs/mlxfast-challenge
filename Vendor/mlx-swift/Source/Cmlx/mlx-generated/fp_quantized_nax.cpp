...
[[kernel]] void fp_gather_qmm_rhs_expert_chain_nax(
  ...
  for (int expert_slot = 0; expert_slot < kExperts / kExpertGroups;
    ...
    for (int chunk_start = bounds[0]; chunk_start < bounds[1];
      ...
      NAXTile<bfloat, TM, TK> activation_tiles[kIntermediate / SK];
      ...
      for (int ct = 0; ct < kGateUpRows / BN; ++ct) {
        ...
        if (sg_active) {
          // The retained source bank is physically [gate32, up32] inside
          // every 64-row output tile. TN=4 therefore maps fragments 0/1 to
          // the gate half and 2/3 to the matching up half.
          constexpr int kHalfFragments = TN / 2;
          STEEL_PRAGMA_UNROLL
          for (int frag = 0; frag < TN; ++frag) {
            ...
            for (int elem = 0;
              ...
              if (frag < kHalfFragments) {
                activation_tiles[ct]
                    .frag_at(0, frag)[elem] = projected;
              } else {
                const bfloat gate =
                    activation_tiles[ct]
                        .frag_at(0, frag - kHalfFragments)[elem];
          ...
          if (sg_active) {
            ...
            for (int kk1 = 0; kk1 < BK; kk1 += SK) {
              NAXTile<bfloat, TM, TK> Atile =
                  activation_tiles[k * (BK / SK) + kk1 / SK];
              NAXTile<Wtype, TN, TK> Btile;
              Btile.template load<Wtype, kBKPad, 1>(
                  Ws + tn * kBKPad + kk1);
              tile_matmad_nax(
