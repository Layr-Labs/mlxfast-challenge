...
[[kernel]] void fp_gather_qmm_rhs_expert_nax(
  ...
  for (int expert_slot = 0; expert_slot < experts / expert_groups;
    ...
    for (int chunk_start = run_start; chunk_start < run_end;
      ...
      for (int k = 0; k < K_it; ++k) {
        threadgroup_barrier(mem_flags::mem_threadgroup);
        // Same bytes, addresses, decode and scale mapping; only access width
        // changes. Stores need no host certification (Ws is 16B aligned by
        // construction); loads are certified by darkbloom_stage_wide_load_ok
        // and self-guarded per thread.
