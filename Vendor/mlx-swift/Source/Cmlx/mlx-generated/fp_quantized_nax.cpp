  ...
  dispatch_bool(aligned_M || !is_unaligned_sm, [&](auto kAlignedM) {
    dispatch_bool(aligned_N || !is_unaligned_bn, [&](auto kAlignedN) {
      for (int k = 0; k < kernel_K; k += BK) {
        // A is read-only and independent of the shared-weight tile. Load both
        // fragments before the barriers so their latency overlaps B staging.
        NAXTile<T, TM, TK> Atile[BK / SK];
        STEEL_PRAGMA_UNROLL
        for (int kk1 = 0; kk1 < BK; kk1 += SK) {
          if constexpr (kAlignedM.value) {
            Atile[kk1 / SK].load(x + kk1, kernel_K);
          } else {
            Atile[kk1 / SK].load_rows(x + kk1, kernel_K, sgp_sm);
          }
        }
