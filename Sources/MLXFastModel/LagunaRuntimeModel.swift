// Thermal-lottery re-roll of the shape-agnostic pair; comment-only delta.
import Foundation
import MLX
import MLXFast
import MLXLMCommon
...
/// Pure Swift-side ranked-shape descriptor plan, built after derived weights
/// are prepared. Set `DARKBLOOM_EXEC_PLAN=0` to retain the dynamic layer loop.
private let lagunaExecutionPlanEnabled =
    ProcessInfo.processInfo.environment["DARKBLOOM_EXEC_PLAN"] == "1"

/// Laguna full-attention caches use direct append on growth and a 128-row
/// quantum. Generic `KVCacheSimple` instances retain their stock behavior.
private let lagunaKVDirectAppendEnabled =
    ProcessInfo.processInfo.environment["DARKBLOOM_KV_DIRECT_APPEND"] != "0"

/// Multi-token fused QK/RoPE consumers receive zero-copy family-atlas slices
/// and an offset of zero. Set to `0` to restore full-atlas + scalar offset.
private let lagunaRoPEAtlasSliceEnabled =
    ProcessInfo.processInfo.environment["DARKBLOOM_ROPE_ATLAS_SLICE"] != "0"
...
let lagunaPackedAffineMetadataEnabled =
    ProcessInfo.processInfo.environment["DARKBLOOM_PACKED_AFFINE_META"] == "1"
...
private func lagunaPrefillSlidingQKNormRoPE(
    ...
    precondition(
        ...
        ] || (lagunaRoPEAtlasSliceEnabled
            && angles.shape == [1, 1, length, LagunaConstants.headDim]))
...
private func lagunaPrefillFullQKNormYaRN(
    ...
    precondition(
        angles.shape == [
            ...
            && angles.shape == [1, 1, length, LagunaConstants.headDim / 2]))
    precondition(offsets.dtype == .int32 && offsets.size == 1)
    precondition((heads + kvHeads) % 4 == 0)
...
enum LagunaRuntimePlannedMLP {
    case dense(LagunaRuntimeMLP)
    case sparse(LagunaRuntimeSparseMoEBlock)
}

enum LagunaRuntimeAttentionFamily: Equatable {
    case full
    case sliding
}

struct LagunaRuntimeLayerExecutionPlan {
    let index: Int
    let layer: LagunaRuntimeDecoderLayer
    let attentionFamily: LagunaRuntimeAttentionFamily
    let mlp: LagunaRuntimePlannedMLP
    let isFinal: Bool
    let decodeAsyncAfter: Bool
    let prefillAsyncAfter: Bool
}

struct LagunaRuntimeExecutionPlan {
    let layers: [LagunaRuntimeLayerExecutionPlan]
}

enum LagunaRuntimePlannedRegime: Equatable {
    case decode
    case prefill
}
...
final class LagunaRuntimeDecoderLayer: Module {
    ...
    func callAsFunction(
        ...
        qkRoPEAngles: MLXArray? = nil,
        qkRoPEOffsets: MLXArray? = nil,
        plannedMLP: LagunaRuntimePlannedMLP? = nil
    ) -> MLXArray {
        let plannedSparse: LagunaRuntimeSparseMoEBlock?
        let plannedDense: LagunaRuntimeMLP?
        switch plannedMLP {
        case .sparse(let sparse):
            plannedSparse = sparse
            plannedDense = nil
        ...
        if lagunaFusedResidualRMSNormRouterEnabled,
            ...
            let sparse = plannedSparse ?? (mlp as? LagunaRuntimeSparseMoEBlock),
        ...
        if let dense = plannedDense ?? (mlp as? LagunaRuntimeMLP),
...
final class LagunaRuntimeModelInner: Module {
    ...
    var _executionPlan: LagunaRuntimeExecutionPlan?
    ...
    /// Resolve layer family, concrete MLP type, final-layer routing and async
    /// boundaries once after derived weights are installed. The plan stores
    /// typed module references in value descriptors; it does not capture
    /// closures or alter any MLX operation.
    func prepareExecutionPlan() {
        guard lagunaExecutionPlanEnabled, _executionPlan == nil,
            layers.count == layerTypes.count
        else {
            return
        }

        var plannedLayers: [LagunaRuntimeLayerExecutionPlan] = []
        plannedLayers.reserveCapacity(layers.count)
        for (index, layer) in layers.enumerated() {
            let family: LagunaRuntimeAttentionFamily
            switch layerTypes[index] {
            case .full:
                guard !layer.selfAttn.isSliding else { return }
                family = .full
            case .sliding:
                guard layer.selfAttn.isSliding else { return }
                family = .sliding
            ...
            let plannedMLP: LagunaRuntimePlannedMLP
            if let dense = layer.mlp as? LagunaRuntimeMLP {
                plannedMLP = .dense(dense)
            } else if let sparse = layer.mlp as? LagunaRuntimeSparseMoEBlock {
                plannedMLP = .sparse(sparse)
            } else {
                return
            }
            ...
            case .layer(let selected):
                decodeAsyncAfter = selected == index
            case .ladder(let stride):
                decodeAsyncAfter = (index + 1) % stride == 0
            case .explicit(let mask):
                decodeAsyncAfter = (mask >> UInt64(index)) & 1 == 1
            case .off, .norm, .logits:
                decodeAsyncAfter = false
            }
            let prefillAsyncAfter =
                lagunaPrefillAsyncLadderStride > 0
                && (index + 1) % lagunaPrefillAsyncLadderStride == 0

            plannedLayers.append(
                LagunaRuntimeLayerExecutionPlan(
                    index: index,
                    layer: layer,
                    attentionFamily: family,
                    mlp: plannedMLP,
                    isFinal: index == layers.count - 1,
                    decodeAsyncAfter: decodeAsyncAfter,
                    prefillAsyncAfter: prefillAsyncAfter
                ))
        }
        _executionPlan = LagunaRuntimeExecutionPlan(layers: plannedLayers)
    }
    ...
    func callAsFunction(_ inputs: MLXArray, cache: [KVCache]? = nil) -> MLXArray {
        let plannedExecution:
            (plan: LagunaRuntimeExecutionPlan, regime: LagunaRuntimePlannedRegime)?
        if lagunaExecutionPlanEnabled, let plan = _executionPlan,
            inputs.dtype == .int32, inputs.ndim == 2,
            inputs.dim(0) == 1, inputs.dim(1) == 1
        {
            plannedExecution = (plan, .decode)
        } else if lagunaExecutionPlanEnabled, let plan = _executionPlan,
            inputs.dtype == .int32, inputs.ndim == 2,
            inputs.dim(0) == 1, inputs.dim(1) > 1
        {
            plannedExecution = (plan, .prefill)
        } else {
                ...
                let slidingAtlas = _slidingRoPEAngleAtlas
            {
                ...
                if graphOffsetArray(for: familyCache) == nil {
                    ...
                    if offset >= 0, offset + length <= lagunaRoPEAngleAtlasLength {
                        if lagunaRoPEAtlasSliceEnabled {
                            // Basic slicing over complete inner rows is a
                            // zero-copy view: no `contiguous` or `eval` is
                            // introduced here. Both family kernels retain
                            // their old `(offset + row)` address expression,
                            // now with a sliced base and literal zero offset.
                            let positions = offset ..< (offset + length)
                            fullRoPEAngles =
                                fullAtlas[0..., 0..., positions, 0...]
                            slidingRoPEAngles =
                                slidingAtlas[0..., 0..., positions, 0...]
                            qkRoPEOffsets = MLXArray([Int32(0)])
                        } else {
        ...
        if let plannedExecution {
            let isDecode = plannedExecution.regime == .decode
            for entry in plannedExecution.plan.layers {
                let mask =
                    entry.attentionFamily == .full ? fullMask : slidingMask
                let qkRoPEAngles =
                    entry.attentionFamily == .full
                    ? fullRoPEAngles : slidingRoPEAngles
                if !isDecode, entry.isFinal {
                    if case .causal = mask {
                        h = entry.layer.callLastPrefillRow(
                            h,
                            cache: cache?[entry.index],
                            plannedMLP: entry.mlp)
                    } else {
                        h = entry.layer(
                            h,
                            mask: mask,
                    ...
                    if isDecode, entry.decodeAsyncAfter {
                        asyncEval(h)
                    }
                    if !isDecode, entry.prefillAsyncAfter {
                        asyncEval(h)
                    }
                }
            }
            ...
            for (i, layer) in layers.enumerated() {
                ...
                if i == layers.count - 1, h.dim(1) > 1 {
                    if case .causal = mask {
                        ...
                        if case .ladder(let n) = lagunaDecodeAsyncStage,
                            (i + 1) % n == 0, inputs.shape == [1, 1]
                        {
                            asyncEval(h)
                        }
                    }
...
public final class LagunaRuntimeModel: Module, LanguageModel {
    ...
    public init(_ config: LagunaConfig) {
        // This model is constructed before the worker protocol starts.
        // Build/certify the process-wide SiLU metadata here so neither
        // certification nor pipeline creation can enter a scored window.
    ...
    public func newCache(parameters _: GenerateParameters?) -> [KVCache] {
        (0..<configuration.numHiddenLayers).map { layerIndex in
            if configuration.layerTypes[layerIndex] == .full {
                ...
                if lagunaKVDirectAppendEnabled {
                    cache.step = 128
                    cache.directAppendGrowth = true
                }
                return cache
            } else {
...
// MARK: - Self-certified BF16 SiLU table (folded from LagunaSiluTable.swift
// to keep the editable-surface file inventory unchanged for static review)
...
/// The exact textual block retained in every stock fused MLP kernel. Table
/// variants are derived from the full stock source by replacing only this
/// block; the stock kernel object and source remain unchanged.
...
func lagunaMakeSiluKernelPair(
    ...
    let tableSource = source.replacingOccurrences(
        of: lagunaStockSiluMetalBlock,
        with: lagunaTableSiluMetalBlock
    )
    precondition(
        tableSource != source,
        "Laguna SiLU table kernel did not find the retained textual block")
...
/// Register the already-materialized table with the quantized Metal backend.
/// The sentinel QMM is evaluated only during initialization; the backend
/// retains this exact input array so expert-aligned prefill binds the same
/// 128 KiB buffer used by the Swift custom kernels.
private func lagunaRegisterSiluTableWithQuantizedBackend(_ table: MLXArray) -> Bool {
    setenv("DARKBLOOM_SILU_TABLE_REGISTER", "1", 1)
    defer { unsetenv("DARKBLOOM_SILU_TABLE_REGISTER") }

    let packed = MLXArray.zeros([1, 8_192], dtype: .uint32)
    let scales = MLXArray.zeros([1, 4_096], dtype: .uint8)
    let ticket = quantizedMM(
        table.reshaped([1, 65_536]),
        packed,
        scales: scales,
        biases: nil,
        transpose: true,
    ...
    return ticket.item(Float.self) == 1.0
}

/// Build and certify all 65,536 BF16 inputs before the worker protocol starts.
/// The stock MLX SiLU result is the table. A retained textual kernel and the
/// exact integer-indexed lookup are both compared to it as raw ushort bits on
/// device, so NaNs, infinities, subnormals, and signed zero are covered.
func prepareLagunaSiluTable() {
    guard !LagunaSiluTableState.attempted else { return }
    LagunaSiluTableState.attempted = true
    // Benched after two ranked failures: retain the package for explicit
    // probes only, with the incumbent consumer kernels restored by default.
    guard ProcessInfo.processInfo.environment["DARKBLOOM_SILU_TABLE"] == "1"
    else {
    ...
    if bitCertified {
        eval(stock)
        active = lagunaRegisterSiluTableWithQuantizedBackend(stock)
    }
    if active {
    ...
    if active {
        status = "active"
    } else if bitCertified {
        status = "backend registration failed; stock fallback"
    } else {
        status = "certificate mismatch; stock fallback"
    }
    let line = String(
        format:
