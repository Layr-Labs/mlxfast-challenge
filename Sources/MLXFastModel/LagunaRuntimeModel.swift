...
/// Multi-token prefill served the stock separate banks until the prefill
/// branch below was added; see `lagunaPrefillFusedSharedGateUpEnabled`.
let lagunaFusedSharedGateUpEnabled =
    ProcessInfo.processInfo.environment["DARKBLOOM_FUSED_SHARED_GATE_UP"] != "0"

/// `DARKBLOOM_PREFILL_FUSED_SHARED_GATE_UP` (default on; set "0" to ablate
/// inside the same binary): serve MULTI-TOKEN (prefill) shared-expert gate/up
/// from the same retained row-concatenated NVFP4 `[gate; up]` bank the decode
/// branch already uses. The bank is built unconditionally at init and was
/// already resident; only the prefill dispatch changes.
///
/// This is NOT a dispatch-count optimization -- it moves a split-K threshold.
/// The shared expert is hidden 2048 -> intermediate 512, so the stock prefill
/// pair is two M=512,N=512,K=2048 quantized matmuls. `quantized.cpp:826-831`
/// computes `n_tiles=16, m_tiles=16, current_tgs=256`, hence
/// `split_k = max(1, 512/256) = 2`, and the pair lands in the 32x32-tile,
/// 128-thread `qmm_splitk` fused arm. Concatenating the banks makes N=1024, so
/// `current_tgs` reaches 512, `split_k` collapses to 1, and `:844` falls
/// through to `qmm_nax`'s 64x64 static-shape kernel -- the SAME kernel the
/// shared DOWN projection (K=512, N=2048) already takes today.
///
/// The two are directly comparable and the gap is the whole point: a GPU-time
/// census of the timed prefill window measured `nvfp4_qmm_t_splitk_fused`
/// (grid.x 16) at 119.0 us x 76 dispatches against
/// `nvfp4_qmm_t_nax_static` (grid.x 32) at 26.6 us x 38 -- 9.0 vs 40.4
/// TFLOP/s on identical FLOPs and identical weight bytes.
///
/// Quantized values are untouched (row concatenation of two group-16 4-bit
/// NVFP4 banks is a pure relayout), but the KERNEL changes, so the reduction
/// order changes and this is NOT bit-exact. It is gated on the correctness
...
/// Acceptance-band chunking knob: sparse layers with `layerIdx` below this
/// limit serve prefill from the fused bank, the rest keep the stock pair.
///
/// SHIPPED AT 20 OF 39 SPARSE LAYERS -- roughly half -- deliberately.
/// A GPU-time census A/B of the timed prefill window measured the WHOLE-model
/// change at 113.6/113.7 ms busy (fused) vs 122.3 ms (stock), i.e. **-7.1%**,
/// with `nvfp4_qmm_t_splitk_fused` absent from the fused arm entirely and
/// `nvfp4_qmm_t_nax_static` rising from 38 to 76 dispatches (1245 -> 3273 us).
/// Net -7.05 ms of GPU work. But the ranked acceptance band requires
/// `prefill_speedup` in [0.952, 1.053] against the pinned calibration
/// reference, so shipping all 39 layers at ~1.071 would fail with
/// `acceptance_band_failed` (docs/benchmark-window-freeze.md). Half lands at
/// ~3.5%, inside the band and still ~7x the ~0.5% submission bar; the
/// remainder is a clean follow-up submission by raising this to 40.
///
/// Local wall-clock could NOT resolve this change (two interleaved batches
/// disagreed in sign) because this box currently shows batch-level prefill
/// regime contamination -- the same binary and config measures ~2.0e-4 and
/// ~5.1e-4 s/token in different batches with the cool gate passing at 33-38C
/// in both. Window GPU-busy is immune to that and is what the numbers above
/// are measured against.
let lagunaPrefillFusedSharedGateUpLayerLimit =
    ProcessInfo.processInfo.environment["DARKBLOOM_PREFILL_FUSED_SHARED_GATE_UP_LAYERS"]
    .flatMap(Int.init) ?? 20
...
final class LagunaRuntimeMLP: Module, UnaryLayer {
    ...
    /// Set once by the init-time bank preparation loop. Default false so a
    /// shared expert whose bank was never prepared can never take the prefill
    /// branch, and so the band chunking limit is honored per layer.
    var _prefillFusedGateUpEnabled = false
    ...
    func callAsFunction(_ x: MLXArray) -> MLXArray {
        ...
        if x.dim(1) > 1,
            ...
            _fusedGateUpSplit == LagunaConstants.sharedExpertIntermediateSize
        {
            // Same literals as the decode branch above and as
            // `QuantizedLinear.callAsFunction` (transpose, group 16, 4-bit,
            // .nvfp4, no affine biases, no bias add); `prepareFusedSharedGateUp`
            // pins them. Each quantized output row is computed independently, so
            // the gate and up halves carry the values the separate banks would
            // -- but the KERNEL changes (qmm_splitk -> qmm_nax static), so the
            // reduction order changes and this is NOT bit-exact.
        ...
        for layer in model.layers {
            ...
            if let sparse = layer.mlp as? LagunaRuntimeSparseMoEBlock {
                if lagunaFusedSharedGateUpEnabled {
                    ...
                    sparse.sharedExpert._prefillFusedGateUpEnabled =
                        lagunaPrefillFusedSharedGateUpEnabled
                        && layer.selfAttn.layerIdx < lagunaPrefillFusedSharedGateUpLayerLimit
                }
