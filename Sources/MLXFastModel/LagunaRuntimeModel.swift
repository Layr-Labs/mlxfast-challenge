...
///
/// DEFAULT OFF (restored from promoted submission 796ebd80 / commit 6b1666b,
/// +0.98% ranked, which the 1ba2bf6 whole-file overwrite reverted): the
/// merge is a dependency-chain pessimisation on the decode step. The routed
/// half consumes `inds`, so the merged dispatch cannot be enqueued until the
/// top-8 kernel retires -- but the shared half consumes only `x`, which is
/// ready the moment `residual+rmsnorm+router` lands. Merging therefore parks
/// the shared expert's gate/up work behind a dispatch it has no data
/// dependency on. Splitting spends one extra dispatch per sparse layer to
/// expose that work for overlap. Splitting is arithmetically inert: each
/// half keeps its own kernel, its own row mapping, and its own accumulation
/// order -- the routed rows go through `lagunaRoutedSwiGLUQMV` and the
/// shared rows through `lagunaSharedSwiGLUQMV`, both of which are the same
/// kernels the merged form was built from.
