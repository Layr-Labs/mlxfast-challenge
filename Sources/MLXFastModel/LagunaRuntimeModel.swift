                ...
                escapeBytes.withUnsafeMutableBytes { escPtr in
                    ...
                    for r in 0..<rows {
                        ...
                        if spanOK && pairwiseOK {
                            base[r] = 0x00
                            esc[r] = 0
                            // v7: lane-major for o_proj (groups>64) vs global for q/k/v
                            if groups > 64 {
                                let groupsPerLane = groups / 32
                                let packedPerLane = groupsPerLane / 2
                                for lane in 0..<32 {
                                    for k in 0..<packedPerLane {
                                        let sIdx = lane * groupsPerLane + 2*k
                                        let dIdx = lane * packedPerLane + k
                                        dst[r * (groups/2) + dIdx] = src[rowBase + sIdx]
                                    }
                                }
                            } else {
                                for k in 0..<(groups/2) {
