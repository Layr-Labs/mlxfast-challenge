...
private func lagunaGatedAffineOProjSource(heads: Int) -> String {
    ...
    for (uint k = 0; k < in_vec_size; k += block_size) {
        ...
        float sum = 0.0f;
        // vec4 loads for the gated input row (the stock fp_qmv_fast's wide
        // load idiom): same per-element multiply and BF16 rounding and the
        // same ascending-i `sum` accumulation order as the scalar loop,
        // 4x fewer load instructions. `xp` is 8B-aligned at every K step
        // (8-element lanes, 256-element blocks).
        const device vec<bfloat, 4>* xv = (const device vec<bfloat, 4>*)xp;
        #pragma unroll
        for (uint i = 0; i < values_per_thread / 4; ++i) {
            const vec<bfloat, 4> xi = xv[i];
            const float v0 = float(bfloat(float(xi[0]) * gate));
            const float v1 = float(bfloat(float(xi[1]) * gate));
            const float v2 = float(bfloat(float(xi[2]) * gate));
            const float v3 = float(bfloat(float(xi[3]) * gate));
            x_thread[4 * i] = v0;
            x_thread[4 * i + 1] = v1;
            x_thread[4 * i + 2] = v2;
            x_thread[4 * i + 3] = v3;
...
private func lagunaGatedAffineOProjNVFP4Source(heads: Int) -> String {
    ...
    for (uint k = 0; k < in_vec_size; k += block_size) {
        float gate = gate_table[column >> head_shift];
        // vec4 loads for the gated input row (the stock fp_qmv_fast's wide
        // load idiom): same per-element multiply and BF16 rounding as the
        // scalar loop, 4x fewer load instructions. `xp` is 8B-aligned at
        // every K step (16-element lanes, 512-element blocks).
        const device vec<bfloat, 4>* xv = (const device vec<bfloat, 4>*)xp;
        #pragma unroll
        for (uint i = 0; i < values_per_thread / 4; ++i) {
            const vec<bfloat, 4> xi = xv[i];
            x_thread[4 * i] = float(bfloat(float(xi[0]) * gate));
            x_thread[4 * i + 1] = float(bfloat(float(xi[1]) * gate));
            x_thread[4 * i + 2] = float(bfloat(float(xi[2]) * gate));
            x_thread[4 * i + 3] = float(bfloat(float(xi[3]) * gate));
        }

        for (uint row = 0; row < results_per_simdgroup; ++row) {
            const device uint32_t* wl = ws + row * (in_vec_size / 8);
            // Decode the E4M3 scale for this 16-value group (carry
            // sign-fold: for sbits = 128 + m the carry yields
            // 0x8000 | (m << 7), which IS the half negation including
            // -0.0h; for sbits < 128 the added term is zero -- bit-exact,
            // promoted in `b38f18c`).
            uint8_t sbits = sc[row * in_vec_size_g];
            ushort sraw = ushort(sbits + (sbits & 128)) << 7;
            float scale = float(as_type<half>(sraw)) * 4194304.0f;
            // Decode the two uint32 code words (16 values) via split-nibble
            // and dot with the gated input, accumulating in FP32. The dead
            // +0.0f seed is elided by seeding the first four-term product
            // group directly: +0.0f + t == t bitwise except t == -0.0f, and
            // that case leaves only a sign-of-zero difference the
            // +0.0f-seeded row accumulator absorbs (+0.0f + -0.0f == +0.0f).
            float accum;
            {
                const uint c = wl[0];
                \(extract)
