...
///
/// Exactness. The norm prologue is the same shared `lagunaNormReductionTail`
/// shape already emitted by three other decode kernels in this file
/// (`lagunaResidualRMSNormRouterKernel`, `lagunaResidualRMSNormKernel`,
...
/// `normalization.cpp` thread-count derivation picks for this row
/// (`Vendor/mlx-swift/.../normalization.cpp:56-77`,
/// `Vendor/mlx-swift/.../kernels/rms_norm.metal:12-80`). `eps` is hardcoded to
/// `1.0e-6f` because Laguna's config loader enforces
/// `rms_norm_eps == LagunaConstants.rmsNormEpsilon (1e-6)` as a hard invariant
/// (`LagunaConfig.swift:555-557`), exactly as the other fused norm kernels in
/// this file already assume. This reuses `lagunaNormReductionTailQKV` (not
/// the zero-init-elision variant) so this new kernel's zero-init shape stays
/// bit-for-bit the same "_v1" prologue currently shipped by the QKV and
/// router/residual kernels above.
///
/// The GEMV half replicates MLX's own `gemv_axbpy` dispatch for
/// `out_vec=100352, in_vec=2048`
/// (`Vendor/mlx-swift/.../backend/metal/matmul.cpp:1114-1130`): neither the
/// `K<=64` nor `K>=16*out_vec` override fires, so MLX picks `bm=8` (out_vec
/// >=4096), `bn=1`, `sm=1`, `sn=32`, `tm=4`, `tn=4` -- the same `sn`/`tn` as
/// the router fusion's `out_vec=256` case and the gated-output-projection's
/// `out_vec=2048`/`in_vec=8192` case above, because `sn`/`tn` (and therefore
/// the per-lane column stride, the 128-wide block loop, and the
/// `simd_shuffle_down` reduction ladder) only change when the `K<=64` or
/// `K>=16*out_vec` branches fire, which they do not here. Only `bm` (the
/// native simdgroups-per-threadgroup count) differs with `out_vec`, and a
/// row's value does not depend on it: `GEMVKernel::run`
/// (`Vendor/mlx-swift/.../mlx-generated/gemv.cpp:1297-1436`) has
/// `needs_tgp_reduction = (BN>1) = false` for this shape, so each simdgroup
...
func lagunaFusedFinalNormLMHead(
    ...
    // Sixteen simdgroups of four vocabulary rows each; 100352 / 64 = 1568
    // tiles, matching the router fusion's own "sixteen simdgroups of four
    // rows" comment at the same rows_per_group=64 grid shape.
    let tiles = vocab / 64
...
public final class LagunaRuntimeModel: Module, LanguageModel {
    ...
    public func callAsFunction(_ inputs: MLXArray, cache: [KVCache]?) -> MLXArray {
        ...
        // preconditions hold (see `lagunaFusedFinalNormLMHead`'s doc comment
        // for the exactness argument); otherwise normalize and project
        // separately below, byte-identical to the fused path.
