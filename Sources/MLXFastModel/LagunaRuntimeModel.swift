...
/// `lagunaFusedQKVProjectionSource`). Unlike the removed fused-QKV
/// materialization arm, this keeps
...
/// chunks proved. (The suffix-selection probe arm has been removed.)
/// The shipped form selects the FIRST `count`
...
/// (Rationale compacted for the byte budget; the full measurement
/// narrative is in this mechanism's submission note.)
...
final class LagunaRuntimeAttention: Module {
    ...
    /// the fused-QKV arm is enabled. Plain stored property with a leading
...
let lagunaRouterTop8PrologueHeader = """
    METAL_FUNC uint laguna_router_top8_extract_round(
        ...
        // Packed transport: the (ordinal, index) pair crosses each
        // butterfly step as one uint2 instead of two scalar shuffles.
        // simd_shuffle_xor moves bits verbatim, so both components carry
        // identical values from identical lanes; the comparator, its
        // operand order, and the 16/8/4/2/1 step sequence are unchanged.
        // Halves the shuffle count on the router's serial top-8 chain,
        // which the routed QMV waits on.
