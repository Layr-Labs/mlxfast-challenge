...
/// rollout. The first 16-layer prefix pilot failed hidden correctness, so the
/// retry defaults to the deeper affine-only window `16..<32`; `0` restores the
/// prior BF16 prefill projections, and the start / layer-count / single-layer
/// selectors support bounded rollout and forensics.
...
private let lagunaNativeAffinePrefillQKVFirstLayer: Int = {
    let requested = Int(
        ProcessInfo.processInfo.environment[
            "DARKBLOOM_NATIVE_AFFINE_PREFILL_QKV_FROM"] ?? "16") ?? 16
    return min(max(requested, 0), LagunaConstants.numHiddenLayers)
}()
...
private func lagunaUseNativeAffinePrefillQKV(layer: Int) -> Bool {
    guard lagunaNativeAffinePrefillQKVLayerCount > 0 else { return false }
    if let onlyLayer = lagunaNativeAffinePrefillQKVOnlyLayer {
        return layer == onlyLayer
    }
    let end = min(
        lagunaNativeAffinePrefillQKVFirstLayer + lagunaNativeAffinePrefillQKVLayerCount,
        LagunaConstants.numHiddenLayers)
    return layer >= lagunaNativeAffinePrefillQKVFirstLayer && layer < end
}
