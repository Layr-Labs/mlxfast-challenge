...
final class LagunaRuntimeAttention: Module {
    ...
    func prepareNativeAffineQKVWeight() -> [MLXArray] {
        ...
        if lagunaNativeAffineGateEnabled, gatingEnabled, gatePerHead,
            ...
            let g = lagunaNativeAffineWeight(gate.weight, layer: layerIdx),
        ...
        var biases: MLXArray?
        let partBiases = parts.compactMap(\.biases)
        if partBiases.count == parts.count {
            biases = concatenated(partBiases, axis: 0)
        } else if !partBiases.isEmpty {
            // Mixed biased/unbiased parts cannot share one bank — drop the fold.
            return []
        }
