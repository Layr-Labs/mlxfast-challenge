...
// Resubmission of the four-mechanism set previously measured as submission
// effb6eb9 (PR793), which scored 1.912107 against a 1.920204 frontier. That
// run's RAW candidate times beat the frontier on BOTH axes -- decode
// 0.006857023 vs 0.006934839 (-1.12%) and prefill 0.000233577 vs 0.000237182
// (-1.52%) -- and it lost only on the paired session baseline, which across
// nine sampled ranked runs spreads 5.18% on prefill against candidate
// differences well under 1%. No executable expression is changed by this
// comment; its only function is to supply a fresh archive identity so the
// service creates a new validation run for the same executable candidate.
//
// A seven-mechanism superset (submission 509d271e, PR798) was measured after
// that run and was WORSE on raw times: decode +0.65%, prefill +3.11%. The
// three added mechanisms -- zero-copy LHS, prefill router BF16 sidecar and
// packed KV -- are therefore NOT carried here. That is a real negative result
// on mechanisms that had passed M5 correctness but had never been scored:
// correctness-validated does not imply beneficial.
