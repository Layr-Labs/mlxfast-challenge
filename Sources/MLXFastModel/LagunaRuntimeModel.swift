...
/// current-main terminal-prefill path inside the same binary.
...
/// heads each at the final position; the remaining `2 * L` groups cover four
/// key heads for each supplied position. Each simdgroup runs the same 128-wide
/// RMSNorm and plain-RoPE expressions as the current-main full-row kernel.
...
final class LagunaRuntimeAttention: Module {
    ...
    func callLastPrefillRow(
            ...
            let offsets = qkRoPEOffsets
        {
            ...
            queries =
                qNorm(queries.reshaped(B, 1, nHeads, headDim))
                .transposed(0, 2, 1, 3)
            keys =
                kNorm(keys.reshaped(B, L, nKVHeads, headDim))
                .transposed(0, 2, 1, 3)
