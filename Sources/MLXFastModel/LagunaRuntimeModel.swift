...
// vendored reference and specialized by guarded fast paths.
...
/// Decode-only scale-interleaved routed gate/up side bank; code bytes and all
/// arithmetic boundaries remain shared with the stock fused bank.
...
/// Decode-only row-concatenated routed `[gate; up]` bank; prefill keeps its
/// independently selected sorted gather-GEMM path.
...
/// Moves the exact BF16 softplus boundary into the existing affine `g_proj`
/// dispatch, so the NVFP4 output projection consumes one activated gate row
/// without rebuilding the same gate table in each of its 256 threadgroups.
private let lagunaAffineGateSoftplusEnabled =
    ProcessInfo.processInfo.environment["DARKBLOOM_AFFINE_GATE_SOFTPLUS"] != "0"

private func lagunaAffineGateSoftplusSource(heads: Int) -> String {
    """
    constexpr uint K = \(LagunaConstants.hiddenSize);
    constexpr uint group_size = 32;
    ...
    constexpr uint scale_stride = group_size / values_per_thread;
    ...
    const device uint8_t* ws =
        ...
        + out_row * K + simd_lid * values_per_thread;
    const device bfloat* sc =
        scales + out_row * groups_per_row + simd_lid / scale_stride;
    const device bfloat* bs =
        biases + out_row * groups_per_row + simd_lid / scale_stride;
    ...
    for (uint k = 0; k < K; k += block_size) {
        float sum = 0.0f;
        for (uint i = 0; i < values_per_thread; ++i) {
            x[i] = float(input[column + i]);
            sum += x[i];
        }
        for (uint row = 0; row < results_per_simdgroup; ++row) {
            const device uint8_t* wl = ws + row * K;
            float scale = float(sc[row * groups_per_row]);
            float bias = float(bs[row * groups_per_row]);
            float accum = 0.0f;
            for (uint i = 0; i < values_per_thread; ++i) {
                accum += x[i] * wl[i];
            }
    ...
    for (uint row = 0; row < results_per_simdgroup; ++row) {
        ...
        if (simd_lid == 0) {
            ...
            gate_values[out_row + row] = bfloat(gate);
        }
    }
    """
}

private let lagunaAffineGateSoftplusKernels: [Int: MLXFast.MLXFastKernel] = {
    var kernels: [Int: MLXFast.MLXFastKernel] = [:]
    for heads in [LagunaConstants.slidingAttentionHeads, LagunaConstants.fullAttentionHeads] {
        kernels[heads] = MLXFast.metalKernel(
            name: "laguna_affine_gate_softplus_h\(heads)_v1",
            inputNames: ["input", "packed_codes", "scales", "biases"],
            outputNames: ["gate_values"],
            source: lagunaAffineGateSoftplusSource(heads: heads),
            ensureRowContiguous: true)
    }
    return kernels
}()

private func lagunaAffineGateSoftplus(
    input: MLXArray, bank: LagunaNativeAffineWeight, heads: Int
) -> MLXArray? {
    guard lagunaAffineGateSoftplusEnabled,
        bank.mode == .affine, bank.bits == 8, bank.groupSize == 32,
        let biases = bank.biases,
        let kernel = lagunaAffineGateSoftplusKernels[heads],
...
private let lagunaActivatedOProjNVFP4Kernels: [Int: MLXFast.MLXFastKernel] = {
    var kernels: [Int: MLXFast.MLXFastKernel] = [:]
    for heads in [LagunaConstants.slidingAttentionHeads, LagunaConstants.fullAttentionHeads] {
        kernels[heads] = MLXFast.metalKernel(
            name: "laguna_gated_affine_oproj_nvfp4_activated_h\(heads)_v1",
            inputNames: [
...
func lagunaGatedAffineOProjNVFP4(
    ...
    let selectedKernel = gateIsActivated
        ? (lagunaAffineGateSoftplusEnabled
            ? lagunaActivatedOProjNVFP4Kernels[heads] : nil)
        : lagunaGatedAffineOProjNVFP4Kernels[heads]
    guard let kernel = selectedKernel else { return nil }
    let inVec = heads * LagunaConstants.headDim
    let outVec = LagunaConstants.hiddenSize
    guard attentionOutput.dtype == .bfloat16,
        attentionOutput.shape == [1, 1, inVec],
                ...
                let fusedAffine = _nativeAffineQKV
            {
                ...
                if let fusedTailGateLogits {
                    ...
                    if lagunaFusedGatedAffineOProjEnabled,
                        ...
                        let activated = lagunaAffineGateSoftplus(
