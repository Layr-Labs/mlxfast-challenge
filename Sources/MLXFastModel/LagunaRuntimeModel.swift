...
final class LagunaRuntimeSparseMoEBlock: Module, UnaryLayer {
    ...
    func callAsFunction(_ x: MLXArray) -> MLXArray {
            ...
            x.dim(1) == 1, inds.size < 64
        {
            ...
            // inverse gather, router weighting, top-k reduction, routed scale,
            // and shared-expert addition directly, avoiding both the full
            // unsorted expert-output intermediate and a final elementwise
            // dispatch.
            let (sorted, inverseOrder) = switchMLP.callSorted(x, inds)
            return fusedUnsortWeightedSumAndAdd(
                sorted: sorted,
                inverseOrder: inverseOrder,
                weights: weights.asType(sorted.dtype),
                scale: routedScalingFactor,
                residual: sharedExpert(x)
            )
        } else {
