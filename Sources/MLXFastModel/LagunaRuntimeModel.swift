...
struct LagunaNativeAffineWeight {
    ...
    // v12: o_proj lane-major packed scales for group-32 (192/256 groups)
    var packedOProjScales: MLXArray? = nil
    var oProjScaleBase: MLXArray? = nil

    var arrays: [MLXArray] {
        [packedCodes, scales]
            + (biases.map { [$0] } ?? [])
            + (indexedMetadata?.arrays ?? [])
            + (packedOProjScales.map { [$0] } ?? [])
            + (oProjScaleBase.map { [$0] } ?? [])
    }
}

/// v12: lane-major packing for o_proj group-32 (192/256 groups -> 96/128 packed)
private func lagunaPackOProjScales(_ scales: MLXArray) -> (codes: MLXArray, base: MLXArray)? {
    guard scales.dtype == .bfloat16, scales.ndim == 2 else { return nil }
    let rows = scales.dim(0), groups = scales.dim(1)
    guard groups.isMultiple(of: 2), groups>64, rows>0 else { return nil }
    eval(scales)
        ...
        packedBytes.withUnsafeMutableBytes { dstPtr in
            baseBytes.withUnsafeMutableBytes { basePtr in
                ...
                for r in 0..<rows {
                    ...
                    // lane-major: 32 lanes, groupsPerLane = groups/32
                    let gpl = groups/32
                    let ppl = gpl/2
                    var ok=true
                    for lane in 0..<32 {
                        for k in 0..<ppl {
                            let a = src[rowBase + lane*gpl + 2*k]
                            let b = src[rowBase + lane*gpl + 2*k+1]
                    ...
                    if ok {
                        ...
                        for lane in 0..<32 {
                            for k in 0..<ppl {
                                dst[r*(groups/2) + lane*ppl + k] = src[rowBase + lane*gpl + 2*k]
                            }
                        }
                    } else {
                        base[r]=0xFF
...
private func lagunaGatedAffineOProjPackedSource(heads: Int) -> String {
    var src = lagunaGatedAffineOProjSource(heads: heads)
    ...
    src = src.replacingOccurrences(of: "float scale = float(sc[row * in_vec_size_g]);", with: "float scale; uint8_t sb = sbase[row]; if (sb == 0xFF) scale = float(sc[row * in_vec_size_g]); else { uint pIdx = (0); scale = float(psc[row * (in_vec_size_g/2) + pIdx]); }")
    return src
}
...
private let lagunaGatedAffineOProjPackedKernels: [Int: MLXFast.MLXFastKernel] = {
    ...
    for heads in [LagunaConstants.slidingAttentionHeads, LagunaConstants.fullAttentionHeads] {
        kernels[heads] = MLXFast.metalKernel(
            ...
            inputNames: ["attention_output", "gate_logits", "weight_codes", "weight_scales", "weight_biases", "packed_scales", "scale_base"],
            outputNames: ["projected"],
            source: lagunaGatedAffineOProjPackedSource(heads: heads),
            ensureRowContiguous: true
        )
    }
    return kernels
...
final class LagunaRuntimeAttention: Module {
    ...
    func prepareNativeAffineOProjWeight() -> [MLXArray] {
            ...
            preparedWO.groupSize == 32, let biases = preparedWO.biases
        {
            ...
            // v12: lane-major packing for o_proj (192/256 groups -> 96/128)
            if let packed = lagunaPackOProjScales(preparedWO.scales) {
                preparedWO.packedOProjScales = packed.codes
                preparedWO.oProjScaleBase = packed.base
            }
        }
