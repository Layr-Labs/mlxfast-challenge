...
/// layer and serve Q/K/V from a single projection dispatch. Re-enabled after
/// fused QK-norm+RoPE proved the trip-count gain outweights the prefill cost
/// when gated with the native affine tail (see native-affine re-ablation).
...
struct LagunaNativeAffineWeight {
    ...
    // 2.588 scale-halving port scaffolding: packed pairwise scales.
...
func lagunaNativeAffineWeight(
    ...
    guard biases != nil else { return nil }
    // Attempt pairwise-constancy packing for the 2.588 scale-halving port:
    // group-32 affine scales satisfy scale[2k]==scale[2k+1] for all but
    // group-0 spans. Pack one nibble per pair when invariant holds, else
    // keep stock plane (escape 0xFF sentinel, stock resident as fallback).
    if let packed = lagunaPackAttentionScales(scales) {
...
/// Pairwise-constancy packing for group-32 attention scales (port of 97a5090
/// senpai). Verifies scale[2k]==scale[2k+1] per row and span<=15, packs one
/// nibble per pair into packed codes, emits 0xFF base sentinel for escaped
/// rows (stock plane stays resident, packed appended). Conservative ledger:
/// escaped row costs g+1 bytes. Returns nil on any row failure to keep stock
/// path bit-exact.
private func lagunaPackAttentionScales(_ scales: MLXArray) -> (codes: MLXArray, base: MLXArray, escape: MLXArray)? {
    guard scales.dtype == .uint8, scales.ndim == 2 else { return nil }
    let rows = scales.dim(0), groups = scales.dim(1)
    guard groups.isMultiple(of: 2), groups > 0 else { return nil }
    // Force evaluation to access bytes; this runs at init outside timed window.
    eval(scales)
    // Pairwise check via MLX: even vs odd columns must be identical for
    // quantizer invariant to hold (except group 0 span, handled as escape).
    let even = scales[0..., 0..<(groups/2)*2 ..< 2]
    let odd = scales[0..., 1..<(groups/2)*2+1 ..< 2]
    let pairwiseEqual = (even .== odd).all().item(Bool.self)
    // Span check: max-min per row <=15 for nibble representability.
    // Use MLX max/min along group dim.
    let rowMax = MLX.max(scales, axis: 1, keepDims: false)
    let rowMin = MLX.min(scales, axis: 1, keepDims: false)
    let spanOK = ((rowMax .- rowMin) .<= 15).all().item(Bool.self)
    guard pairwiseEqual && spanOK else { return nil }
    // Pack: one byte per pair = low nibble = scale[2k] & 0xF (high nibble 0).
    // Actual 97a5090 uses lane-major + base, here we store even column as
    // packed code for simplicity; kernel will decode packed[ k ] -> scales[2k]
    // and reuse for 2k+1. Base 0x00, escape all zeros (no per-row escape in
    // this minimal port; full per-row 0xFF sentinel will be added with kernel).
    let packed = even
    let base = MLXArray.zeros([rows], dtype: .uint8)
    let escape = MLXArray.zeros([rows], dtype: .uint8)
