...
/// `DARKBLOOM_PREFILL_FUSED_MLP_GATE_UP` (default OFF; set "1" to enable):
/// the shared-expert / dense-L0 prefill fused [gate;up] arm described above.
/// Split from `lagunaPrefillFusedRoutedGateUpEnabled` (the known-good sorted
/// routed fusion) because the MLP arm measured NON-bit-exact on first test
/// (2026-08-02: teacher-forced step-0 token 345 vs main's 8550 on M3 Ultra)
/// — root cause under investigation; keep off until fixed.
...
final class LagunaRuntimeMLP: Module, UnaryLayer {
    ...
    func callAsFunction(_ x: MLXArray) -> MLXArray {
        ...
        // `DARKBLOOM_PREFILL_FUSED_GATE_UP` (shared with the sorted routed
        // prefill fusion): one projection dispatch over the ALREADY-RETAINED
        // row-concatenated [gate; up] bank, then the identical stock split,
        // silu product, and down projection. Bit-exact by the same
        // row-independence argument as the retained QKV bank: each output
        // row's K-loop (and, for the NVFP4 bank, its scale row) reads only
        // its own weight bytes, so the split halves equal the two separate
        // stock projections element for element.
        if lagunaPrefillFusedMLPGateUpEnabled, x.dim(1) > 1,
            x.dtype == .bfloat16, x.dim(2) == LagunaConstants.hiddenSize
        {
            // Shared expert (and any NVFP4 instance): one quantizedMM over
            // the fused bank, mirroring `QuantizedLinear.callAsFunction`'s
            // literals exactly (transpose, group 16, 4-bit, .nvfp4, no
            // biases -- pinned by `prepareFusedSharedGateUp`'s guards).
            ...
            // Dense layer 0 (plain BF16 `Linear`s): one matmul over the
            // fused bank, the same `matmul(x, w.T)` each stock projection
            // performs.
            let intermediate = LagunaConstants.denseIntermediateSize
