...
/// Four SIMDs cover 16 QMV rows; set the environment value to 2 for control.
...
/// Fuses exact RMSNorm with native group-32 affine QKV/G projection. Each
/// threadgroup reproduces the stock 16-partial RMS fold and QMV lane order;
/// inline mode avoids a 4 KB threadgroup staging allocation.
...
/// Register-prefetch depth: 2 for SG4, 4 for SG2, ignored by staged mode.
...
/// Hoists immutable weight metadata loads while preserving arithmetic order.
...
/// One kernel per reachable QKV/G row count.
...
/// Prefill keeps the stock shapeless unary gate.
...
/// Decode-only compiled gate product plus output projection.
