...
/// (`DARKBLOOM_NATIVE_AFFINE_NVFP4_FROM`, default 16), using the same
...
///
/// Window widened 32 -> 16 (this submission). Two independent reasons, one
/// numeric and one for bytes:
///
///  * NUMERICS IMPROVE, they do not degrade. The checkpoint ships these
///    projections as group-16 4-bit NVFP4; the group-32 affine INT8 side
///    layout is a re-quantization DERIVED from them, so INT8 carries strictly
///    more representation error than re-encoding to the shipped format does.
///    Widening the window therefore moves the scored path TOWARD the
///    reference implementation the goldens come from, not away from it —
///    the opposite direction from the usual "perturbation amplifies through
///    the trunk" risk that motivated the original tail-only window. Layers
///    0-15, the highest-amplification third, still keep the long-validated
///    INT8 path, so this is one chunk of a rollout, not the whole rollout.
///
///  * BYTES. NVFP4 g16 is 0.5625 B/weight against INT8 g32's 1.125, and it
///    carries two device planes (codes, scales) instead of three (codes,
///    scales, biases). The attention projections are ~51% of the decode read
///    stream and sit at the DRAM roofline, where byte and stream-count cuts
///    are the levers that pay.
///
/// Honest cost, stated because it is a real trade: a layer inside the window
/// takes the NVFP4 o_proj path, which is two dispatches (softplus
/// gate-product, then the NVFP4 qmv) where the INT8 path's gated fusion is
/// one. Sixteen more layers therefore add ~16 dispatches/token against the
/// byte saving above.
private let lagunaNativeAffineNVFP4From: Int? = {
    guard ProcessInfo.processInfo.environment["DARKBLOOM_NATIVE_AFFINE_NVFP4"] != "0"
    else { return nil }
    let raw = ProcessInfo.processInfo.environment["DARKBLOOM_NATIVE_AFFINE_NVFP4_FROM"] ?? "16"
