...
/// All-family cache proof admits the default-on PC1 dose; unsupported states
/// stay on PC0. The stronger PC2 scale-literal identity remains opt-in so its
/// input-edge deletion is independently attributable and rollbackable.
let lagunaAttentionParamsRoPESidecarEnabled =
    ProcessInfo.processInfo.environment[
        "DARKBLOOM_ATTN_PARAMS_ROPE_SIDECAR"] != "0"
let lagunaAttentionScaleLiteralEnabled =
    ProcessInfo.processInfo.environment["DARKBLOOM_ATTN_SCALE_LITERAL"] == "1"
