...
// PC2 is a sliding-only PC1 refinement; setting this to 0 retains PC1.
...
struct LagunaDecodeAttentionParamsProof {
    ...
    var producerParams: MLXArray {
        MLXArray([
            UInt32(position), 0, 0, UInt32(slidingWriteIdx),
        ])
    }
...
func lagunaFullFusedAttention(
    ...
    precondition(keyWeight.dims(LagunaConstants.headDim))
    precondition(angles.dtype == .float32)
    // PC1 is deliberately sliding-only. Full attention always keeps PC0's
    // ordinary parameter and scale inputs, with its unextended YaRN row.
    precondition(angles.dims(1, 1, 1, LagunaConstants.headDim / 2))
    precondition(cacheKeys.dtype == .bfloat16)
    precondition(
    ...
    lagunaTrace("full fused attention pc0")
    let commonInputs: [any ScalarOrArray] = [
            rawQueries, rawKeys, rawValues,
            queryWeight, keyWeight, angles,
            cacheKeys, cacheValues,
    ...
    return lagunaFullFusedAttentionPC0Kernel(
...
/// Ordinary warm covers full PC0; explicitly warm sliding PC1 and PC2.
func lagunaWarmAttentionParamsRoPESidecarKernels(_ model: LagunaRuntimeModel) {
    ...
    let proof = LagunaDecodeAttentionParamsProof(position: 0, slidingWriteIdx: 1)
    ...
    lagunaTrace("warm full pc0; sliding attention pc1 and pc2")
    eval([produced.hidden, produced.fullAngles, produced.slidingAngles,
        slidingPC1, slidingPC2])
}
    ...
    func callAsFunction(
            ...
            let ring = rotating.fusedRingPrepare()
        {
            ...
            fusedAttended = lagunaSlidingFusedAttention(
                ...
                sidecarProof: isSliding ? attentionParamsProof : nil,
            ...
            let append = simple.fusedAppendPrepare()
        {
            ...
            qkNormRoPEFused = true
        } else if useFusedFullQKNormYaRN, let angles = qkRoPEAngles {
...
final class LagunaRuntimeModelInner: Module {
    ...
    /// Read-only sliding-family certificate; consumer admission rechecks it.
    private func decodeAttentionParamsProof(
        ...
        for (index, layerType) in layerTypes.enumerated() {
            guard layerType == .sliding else { continue }
        ...
        guard let slidingDescriptor else { return nil }
