...
/// Package three exact one-row routed/shared-down owners into one 27-SIMD
/// threadgroup. This changes only the scheduling container: per-row QDOTs,
/// reduction order, bytes, and BF16 boundaries stay unchanged. The default
/// twelve sparse layers chunk the full local win below the ranked 5% band.
...
private let lagunaRoutedSharedDownResidualTG3Kernel = MLXFast.metalKernel(
    name: lagunaSharedFirstDownOrderEnabled
        ? "laguna_routed_shared_nvfp4_down_residual_bf16_tg3_v1sf"
        : "laguna_routed_shared_nvfp4_down_residual_bf16_tg3_v1",
    inputNames: lagunaSharedFirstDownOrderEnabled
        ? [
            "shared_activated", "shared_down_weight", "shared_down_scales",
            "routed_activated", "routed_down_weight", "routed_down_scales",
    ...
    source: """
        ...
        constexpr uint rows_per_group = 3;
        constexpr uint shared_slot = 8;
        constexpr uint values_per_lane = 16;
        constexpr uint packed_row_bytes = 256;
        constexpr uint scale_row_bytes = 32;
...
func lagunaRoutedSharedDownResidual(
    ...
    let threads = threeRowsPerGroup ? 864 : 288
    let groups = threeRowsPerGroup
        ? (LagunaConstants.hiddenSize + 2) / 3
        : LagunaConstants.hiddenSize
    return kernel(
    ...
    private func forward(
            ...
            x.dim(1) == 1, inds.size < 64
        {
                ...
                residual.shape == [1, 1, LagunaConstants.hiddenSize]
            {
                ...
                lagunaTrace(
                    threeRowsPerGroup
                        ? "routed+shared down residual tg3"
