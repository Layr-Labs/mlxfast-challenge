...
/// Group-16 NVFP4 attention tail, widened from layer 24 to layer 16.
///
/// NVFP4 as shipped costs 0.5625 B/param (4-bit codes plus one e4m3 scale
/// byte per 16) against 1.125 B/param for the group-32 affine INT8 side
/// layout (1 B plus a 2 B scale and a 2 B bias per 32), so every layer moved
/// off INT8 HALVES that layer's attention weight traffic. Decode is
/// bandwidth-bound here, and the local sweep is monotone at about -0.5%
/// decode per layer moved (FROM=32 5.614 ms, 28 5.532, 24 5.416, 16 5.154,
/// 0 4.683, teacher-forced 200 steps each).
///
/// Prior tip (polymorf) moved layers 24..31 onto the NVFP4 tail. This chunk
/// moves the next 8 layers (16..23), worth about another -3.5% decode under
/// that sweep, staying inside the ~5% single-submission acceptance band;
/// layers 0..15 remain for later chunks. Numerically this moves TOWARD the
/// reference rather than away from it -- NVFP4 is the shipped representation
/// the goldens were generated from, while the INT8 side layout is the lossy
/// re-quantization -- and it is option (1) of the frozen quantization
/// envelope, which never requires the INT8 re-quant. Set
/// `DARKBLOOM_NATIVE_AFFINE_NVFP4=0` to disable the tail entirely, or
/// `DARKBLOOM_NATIVE_AFFINE_NVFP4_FROM=24` to restore the previous default.
