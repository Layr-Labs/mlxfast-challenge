...
/// the fused-KV-write ranked regression does not apply here.
...
/// over all 65536 patterns), and the 16-bit pattern identity is
/// machine-checked in Lean 4 (`bv_decide`, both the uint-add-then-truncate
/// and direct 16-bit readings pinned against each other). Applies only with
/// the scale fold ON; the fold-OFF ablation arm keeps the stock body.
