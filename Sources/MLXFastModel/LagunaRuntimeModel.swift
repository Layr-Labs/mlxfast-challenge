...
///
/// Correctness is an exact bound: every consumer still receives an MLXArray
/// whose single UInt32 payload equals the write index (and for full layers,
/// N = writeIdx+1 and capacity). Sharing reuses that host literal when the
/// key matches across layers of the same decode step; a divergent key always
/// allocates a fresh literal identical to the per-call path.
