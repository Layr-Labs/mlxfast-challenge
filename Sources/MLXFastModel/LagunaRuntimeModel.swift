...
private let lagunaNativeAffineNVFP4From: Int? = {
    ...
    let raw = ProcessInfo.processInfo.environment["DARKBLOOM_NATIVE_AFFINE_NVFP4_FROM"] ?? "17"
...
// Decode router top-8 prologue: twin of packed routed gate/up QMV that
// selects its expert in-kernel from router logits (overlay restore of
// 3223e19 / DARKBLOOM_ROUTER_TOP8_PROLOGUE). Bit-identical top-8 via
// laguna_router_ordinal_before; joins the QMV barrier group so the
// standalone top-8 window can drop per sparse layer on decode.
// CRITICAL: correction_bias must be float32 end-to-end (checkpoint F32
// [256]). BF16 truncation reorders near-ties vs the standalone gate()
// inds consumed by down-proj / mixture weights → expert cross-wire.
// Standalone top-8 still runs for those consumers; prologue only removes
// the QMV's data dependency on indices.
/// Default ON; set DARKBLOOM_ROUTER_TOP8_PROLOGUE=0 to disable.
...
func lagunaRoutedSwiGLUQMVPackedTop8(
    ...
    // Must match standalone gate/top-8 paths: checkpoint bias is F32 and
    // down-proj inds are selected with float32 bias. BF16 truncation here
    // can reorder near-ties and cross-wire experts vs mixture weights.
    precondition(correctionBias.dtype == .float32)
    precondition(correctionBias.size == LagunaConstants.numExperts)

    let outputs = lagunaRoutedSwiGLUQMVPackedTop8Kernel(
...
/// Selection-only twin of the packed QMV top-8 prologue for parity tests.
/// Uses the same ordinal key + extract_round schedule as
/// `laguna_routed_nvfp4_swiglu_qmv_packed_top8p_bf16_v1`, with float32 bias.
private let lagunaRouterTop8PrologueIndicesKernel = MLXFast.metalKernel(
    name: "laguna_router_top8_prologue_indices_v1",
    inputNames: ["logits", "correction_bias"],
    outputNames: ["router_indices"],
    source: """
        uint lane = thread_position_in_threadgroup.x;
        threadgroup uint top8_keys[256];

        // 256 lanes: one expert key each (same formula as QMV prologue).
        float x = float(logits[lane]);
        float score = laguna_router_top8_score(x);
        float key = -(score + float(correction_bias[lane]));
        top8_keys[lane] = laguna_router_key_ordinal(key);
        threadgroup_barrier(mem_flags::mem_threadgroup);
...
/// Returns the eight expert indices the QMV top-8 prologue would select, in
/// rank order (slot 0 = best). For unit tests only — not on the scored path.
func lagunaRouterTop8PrologueIndicesForTesting(
    ...
    let outputs = lagunaRouterTop8PrologueIndicesKernel(
        [logits, correctionBias],
        grid: (256, 1, 1),
        threadGroup: (256, 1, 1),
        outputShapes: [[1, 1, 8]],
                    ...
                    let packedBank = _packedRoutedGateUpBank
                {
                        ...
                        gate.eScoreCorrectionBias.size == LagunaConstants.numExperts
                    {
                        ...
                        activated = lagunaRoutedSwiGLUQMVPackedTop8(
                            ...
                            logits: logits,
                            correctionBias: gate.eScoreCorrectionBias.asType(.float32)
                        )
                    } else {
