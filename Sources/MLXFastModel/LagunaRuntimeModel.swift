...
///    so the redundant reads add no DRAM traffic against the ~20 MB bank read.
...
///    logit the stock qmv writes). Every output row's K loop is independent of
///    which rows share the dispatch, so every Q/K/V/gate element is
///    bit-identical to the two-dispatch chain.
private func lagunaFusedNormAffineQKVSource(heads: Int) -> String {
    ...
    // 16 simdgroups = 8 tiles of 2; tile u owns rows u*8 + sub*4, offset by
    // this threadgroup's 64-row block base.
    ...
    func callAsFunction(
            ...
            gateProjection.weight.shape == [nHeads, LagunaConstants.hiddenSize]
        {
                ...
                let fusedAffine = _nativeAffineQKV
            {
                ...
                // When the gate rides the bank (INT8 layers) and this is not
                // layer 0's precomputed-norm handoff, fold the input RMSNorm
                // into the QKV qmv as one dispatch. Bit-identical to the
                // two-dispatch chain; `normalized` then has no other consumer
                // on this path (the gate is sliced from `qkv` below).
                let fusedNormAffineQKV: MLXArray? =
                    precomputedNorm == nil && _nativeAffineQKVGateRows == nHeads
                ...
                if let fusedNormAffineQKV {
                    ...
                    normalized = precomputedNorm ?? inputNorm(input)
                    qkv = quantizedMM(
                        normalized,
