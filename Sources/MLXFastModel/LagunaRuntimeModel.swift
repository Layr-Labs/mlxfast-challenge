...
/// QMV, including the packed-scales / top8keys hot path. Official submission
/// `b56a6d9` proved the non-packed R1 arm (1,344 exact-token checks). The
/// packed twins keep the historical 128-tile scale bank and remap each
/// one-row tile onto it, so only occupancy changes. Set `DARKBLOOM_QMV_R1=0`
/// to restore the two-row control on both packed and unpacked arms.
...
/// (simd_group*2 + row)*2 + {0 gate, 1 up}` under the historical R2 schedule.
/// The R1 arm (`DARKBLOOM_QMV_R1`, default ON) keeps this bank and remaps
/// each one-row tile onto it; only threadgroup geometry changes.
...
/// Packed routed QMV body generator.
///
/// `rowsPerSimd == 2` is the historical 128-tile schedule (two rows per
/// simdgroup). `rowsPerSimd == 1` is the R1 occupancy twin already proven on
/// the non-packed routed QMV (`DARKBLOOM_QMV_R1`): 256 tiles, one row per
/// simdgroup. The packed scale side bank stays in the historical 128-tile
/// walk order from `preparePackedRoutedGateUpBank`; the R1 arm maps
/// `logical_row` back onto that bank with `bank_tile = logical_row / 4` and
/// `row_in_bank = logical_row % 4`, so scale bytes are identical and no
/// bank rebuild is required. Gate/up code rows, K-block order, qdot calls,
/// BF16 SwiGLU boundaries, and output addresses are unchanged per row.
func lagunaRoutedSwiGLUQMVPackedSelectedSource(
    prologue: String, expertExpression: String, rowsPerSimd: Int = 2
) -> String {
    precondition(rowsPerSimd == 1 || rowsPerSimd == 2)
    if rowsPerSimd == 1 {
        return """
        constexpr uint input_width = 2048;
        constexpr uint output_width = 512;
        ...
        uint lane = thread_index_in_simdgroup;
        uint logical_row = tile * 2 + simd_group;
        uint bank_tile = logical_row / 4;
        uint row_in_bank = logical_row % 4;
        \(prologue)
        ...
        for (uint block = 0; block < input_width; block += block_width) {
            ...
            const device uint8_t* gate_scale =
                block_scales + row_in_bank * 2 * scale_row_bytes + lane;
...
private let lagunaRoutedSwiGLUQMVPackedTop8Rows1Kernel = MLXFast.metalKernel(
    name: "laguna_routed_nvfp4_swiglu_qmv_packed_top8keys_rows1_bf16_v1",
    inputNames: ["input", "fused_weight", "packed_scales", "router_keys"],
    outputNames: ["activated"],
    source: lagunaRoutedSwiGLUQMVPackedSelectedSource(
        prologue: lagunaRouterTop8PrecomputedPrelude,
...
func lagunaRoutedSwiGLUQMVPackedTop8(
    ...
    let kernel =
        rows1
        ? lagunaRoutedSwiGLUQMVPackedTop8Rows1Kernel
