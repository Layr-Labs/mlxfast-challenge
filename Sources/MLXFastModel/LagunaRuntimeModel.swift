...
/// `DARKBLOOM_PREFILL_SHARED_GATE_UP` (default on; set "0" to disable): use
/// the same retained row-concatenated bank for one multi-token gate/up QMM.
/// Output rows remain independent, so this preserves the two projections'
/// per-row NVFP4 arithmetic while deleting one dispatch.
...
final class LagunaRuntimeMLP: Module, UnaryLayer {
    ...
    func callAsFunction(_ x: MLXArray) -> MLXArray {
        ...
        if lagunaPrefillSharedGateUpEnabled, x.dim(1) > 1,
            let fusedWeight = _fusedGateUpWeight,
            let fusedScales = _fusedGateUpScales,
