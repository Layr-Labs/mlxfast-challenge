...
private let lagunaMarkedSortedExpertIndicesEnabled: Bool = {
    let environment = ProcessInfo.processInfo.environment
    let stageVariant = environment["DARKBLOOM_STAGE_BM128"] ?? ""
    return environment["DARKBLOOM_EXPERT_ALIGNED_GATHER"] != "0"
        && (stageVariant.isEmpty || stageVariant == "4" || stageVariant == "5")
        && GPU.deviceInfo().architecture.lowercased().contains("applegpu_g17")
}()

private let lagunaSortedXLHSIndicesEnabled =
    lagunaMarkedSortedExpertIndicesEnabled
    && ProcessInfo.processInfo.environment["DARKBLOOM_SORTED_X_LHS_INDICES"] != "0"
...
private func lagunaFusedSortedRoutedGateUp(
    ...
    var gateUpIndices = idx
    let usesMarkedExpertIndices = lagunaMarkedSortedExpertIndicesEnabled && doSort
        && indices.size >= 4 * LagunaConstants.numExperts
        && x.dim(1) <= 65_536
        && x.dtype == .bfloat16 && x.shape == [1, x.dim(1), LagunaConstants.hiddenSize]
        && indices.dtype == .uint32
        && indices.shape == [1, x.dim(1), LagunaConstants.numExpertsPerTok]
    let useUnsortedLHS =
        lagunaSortedXLHSIndicesEnabled && usesMarkedExpertIndices
    if useUnsortedLHS {
        let s = gatherSortMetadata(indices: indices)
        precondition(s.rowOrder.dtype == .uint32 && s.rowOrder.size == indices.size)
        // DB packs expert bits 23:16 and the original token row in bits 15:0.
        gateUpIndices = (
            UInt32(0xDB00_0000)
                | (s.sortedKeys << UInt32(16))
                | s.rowOrder
        ).reshaped(indices.shape)
        idx = s.sortedKeys
        inverseOrder = s.inverseOrder
    } else if doSort {
        (sortedX, idx, inverseOrder) = gatherSort(x: sortedX, indices: indices)
        if usesMarkedExpertIndices {
            // DA packs expert bits 23:16; materialized rows use output position.
            gateUpIndices =
                UInt32(0xDA00_0000) | (idx << UInt32(16))
        } else {
            gateUpIndices = idx
        }
    }
    // One gather over tile-interleaved gate/up banks replaces the two stock
    // projections without changing their quantization or routing arguments.
    let gateUpRaw = MLX.gatherQuantizedMM(
        sortedX,
        fusedWeight,
        scales: fusedScales,
        biases: nil,
    ...
    let gateUp =
        usesMarkedExpertIndices
        ? gateUpRaw.reshaped([indices.size, 1, 2 * split])
        : gateUpRaw
    let activated: MLXArray
    if usesMarkedExpertIndices {
