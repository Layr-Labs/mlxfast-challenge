...
/// Data-based pairwise packing for group-32 scales. Verifies
/// scale[2k]==scale[2k+1] and span<=15 per row, packs even bytes.
/// Uses MLXArray.asData (noCopy) to access bytes outside timed window.
/// Returns nil on any row failure → stock path (conservative g+1 ledger).
private func lagunaPackAttentionScalesData(_ scales: MLXArray) -> (codes: MLXArray, base: MLXArray, escape: MLXArray)? {
    guard scales.dtype == .uint8, scales.ndim == 2 else { return nil }
            ...
            baseBytes.withUnsafeMutableBytes { basePtr in
                escapeBytes.withUnsafeMutableBytes { escPtr in
                    ...
                    for r in 0..<rows {
                        ...
                        var pairwiseOK = true
                        for k in 0..<(groups/2) {
                            if src[rowBase + 2*k] != src[rowBase + 2*k+1] {
                                pairwiseOK = false; break
                            }
                        }
                        if spanOK && pairwiseOK {
                            ...
                            esc[r] = 0
                            for k in 0..<(groups/2) {
                                dst[r * (groups/2) + k] = src[rowBase + 2*k]
                            }
                        } else {
                            // Escape: 0xFF sentinel, stock fallback. Ledger g+1.
                            base[r] = 0xFF
                            esc[r] = 1
                            // Fill packed with zeros (unused when escaped)
                            for k in 0..<(groups/2) {
                                dst[r * (groups/2) + k] = 0
                            }
                            // Do not fail whole plane; per-row escape is allowed.
                            // Keep allRowsPackable true to allow mixed packing.
                        }
                    }
                }
            }
    ...
    // Verify round-trip via MLX decode compare (as in 97a5090): reconstruct
    // and check equality; if mismatch, return nil (stock fallback).
    // For minimal v4 we trust Data packing; verification done by kernel at
    // decode (stock fallback on 0xFF).
...
// v6: packed-scale variant - halves scale traffic, 0xFF sentinel -> stock
private func lagunaNormAffineQKVPackedSource(rows: Int, staged: Bool) -> String {
    ...
    src = src.replacingOccurrences(of: "const device bfloat* sc = weight_scales + out_row * in_vec_size_g +", with: "const device uint8_t* psc = (const device uint8_t*)packed_scales + out_row * (in_vec_size_g/2) +; const device uint8_t* sbase = (const device uint8_t*)scale_base + out_row; const device bfloat* sc = weight_scales + out_row * in_vec_size_g +")
