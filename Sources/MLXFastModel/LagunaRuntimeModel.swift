...
final class LagunaRuntimeMLP: Module, UnaryLayer {
    ...
    func callAsFunction(_ x: MLXArray) -> MLXArray {
        ...
        if x.dim(1) > 1, lagunaPrefillSharedFusedGateUpEnabled,
            ...
            _fusedGateUpSplit == LagunaConstants.sharedExpertIntermediateSize
        {
            // One quantized matmul over the retained row-concatenated bank
            // replaces the stock separate gate/up prefill dispatches. The
            // preparation contracts pin transpose, group 16, 4-bit NVFP4,
            // and no affine biases; output rows remain independent, so each
            // split half preserves the stock row arithmetic.
