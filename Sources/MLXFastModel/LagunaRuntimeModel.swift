...
/// Sort each lane's eight router candidates once, then merge the 32 sorted
/// lane streams for the requested routed slot. This replaces repeated local
/// rescans without adding a threadgroup barrier or changing the existing
/// `(ordinal, expert index)` total order. Keep it as an explicit opt-in.
private let lagunaRoutedGateUpSortedTop8Enabled =
    ProcessInfo.processInfo.environment["DARKBLOOM_ROUTED_GATEUP_SORTED_TOP8"] == "1"

/// Rescan slots zero through four, then amortize the sorted lane streams over
/// slots five through seven. Set to "0" for the all-rescan control.
...
private let lagunaRoutedSwiGLUQMVPackedTop8R1Kernel = MLXFast.metalKernel(
    name: lagunaRoutedGateUpSortedTop8Enabled
        ...
        : lagunaRoutedGateUpHybridTop8Enabled
            ? "laguna_routed_nvfp4_swiglu_qmv_packed_top8keys_r1_hybrid5_bf16_v1"
...
private let lagunaRoutedSwiGLUQMVPackedTop8R1RouterKernel = MLXFast.metalKernel(
    name: lagunaRoutedGateUpSortedTop8Enabled
        ...
        : lagunaRoutedGateUpHybridTop8Enabled
            ? "laguna_routed_nvfp4_swiglu_qmv_packed_top8keys_r1_router_hybrid5_bf16_v1"
