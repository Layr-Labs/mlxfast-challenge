...
/// different outputs, and share an identical one-row-per-SIMD tile: four
/// SIMD groups cover four output rows across four 512-wide K blocks. Splitting
/// the old two-row accumulator preserves every output row's K-loop, scale
/// application, casts, and SwiGLU arithmetic while a 128-thread group reduces
/// scheduling overhead relative to the two-SIMD r1 form. Merging routed slots
/// 0-7 with shared slot 8 still removes one dispatch per sparse layer.
...
/// Two output rows per SIMD retains the exact row-local NVFP4 dot products
/// and nine-slot BF16 reduction while exposing twice as many independent
/// weight streams as the original four-accumulator form.
private let lagunaRoutedSharedDownResidualKernel = MLXFast.metalKernel(
    name: "laguna_routed_shared_nvfp4_down_residual_bf16_r2_v2",
    inputNames: [
