...
/// The merged routed/shared gate/up kernel uses the measured R2 schedule by
/// default. The R4 control is enabled only when the selector is literally `4`.
/// Each row's arithmetic is unchanged; R4 only trades register pressure
/// against occupancy.
