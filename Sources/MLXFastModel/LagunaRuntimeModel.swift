...
constexpr uint hidden = 2048;
constexpr uint experts = 8;
// Two columns per thread rather than four: halves per-thread accumulator
// and expert-load state and doubles the resident thread count for the same
// work. Purely a work-partition constant -- it never appears in an
// accumulation across columns, so every output element keeps its own
// e-ascending sum and the result is bit-identical.
constexpr uint n_cols = 2;
...
private func lagunaPrefillSortedMoETail(
    ...
    return lagunaPrefillSortedMoETailKernel(
        ...
        grid: (LagunaConstants.hiddenSize / 2, rows, 1),
        threadGroup: (256, 1, 1),
        outputShapes: [[1, rows, LagunaConstants.hiddenSize]],
        outputDTypes: [.bfloat16]
    )[0]
