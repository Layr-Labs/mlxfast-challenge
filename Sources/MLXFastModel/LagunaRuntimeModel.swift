...
struct LagunaNativeAffineWeight {
    ...
    // v4: pairwise-packed attention scales (group-32 invariant).
    var packedAttentionScales: MLXArray? = nil
    var attentionScaleBase: MLXArray? = nil
    var attentionScaleEscape: MLXArray? = nil

    var arrays: [MLXArray] {
        [packedCodes, scales]
            + (biases.map { [$0] } ?? [])
            + (indexedMetadata?.arrays ?? [])
            + (packedAttentionScales.map { [$0] } ?? [])
            + (attentionScaleBase.map { [$0] } ?? [])
            + (attentionScaleEscape.map { [$0] } ?? [])
    }
}
...
func lagunaNativeAffineWeight(
    ...
    guard biases != nil else { return nil }
    // v4: attempt pairwise packing for group-32 (senpai 2.588). Stock plane
    // stays resident, packed appended, 0xFF sentinel for escapes.
    if let packed = lagunaPackAttentionScalesData(scales) {
        ...
        w.packedAttentionScales = packed.codes
        w.attentionScaleBase = packed.base
        w.attentionScaleEscape = packed.escape
        return w
    }
    return LagunaNativeAffineWeight(
        packedCodes: packedCodes,
...
/// Data-based pairwise packing for group-32 scales. Handles both
/// uint8 and bfloat16 scale planes (affine INT8 uses bfloat16 scales).
/// Verifies scale[2k]==scale[2k+1] and span<=15 per row, packs even entries.
/// Uses MLXArray.asData (noCopy) outside timed window. Per-row 0xFF sentinel,
/// stock stays resident, packed appended. Returns nil on whole-plane failure.
private func lagunaPackAttentionScalesData(_ scales: MLXArray) -> (codes: MLXArray, base: MLXArray, escape: MLXArray)? {
    guard scales.ndim == 2 else { return nil }
    let rows = scales.dim(0), groups = scales.dim(1)
    guard groups.isMultiple(of: 2), rows > 0 else { return nil }
    eval(scales)
    if scales.dtype == .uint8 {
        let mlxData = scales.asData(access: .noCopy)
        guard mlxData.data.count == rows * groups else { return nil }
        var packedBytes = Data(count: rows * (groups / 2))
        var baseBytes = Data(count: rows)
        var escapeBytes = Data(count: rows)
        mlxData.data.withUnsafeBytes { srcPtr in
            packedBytes.withUnsafeMutableBytes { dstPtr in
                baseBytes.withUnsafeMutableBytes { basePtr in
                    escapeBytes.withUnsafeMutableBytes { escPtr in
                        let src = srcPtr.bindMemory(to: UInt8.self).baseAddress!
                        let dst = dstPtr.bindMemory(to: UInt8.self).baseAddress!
                        let base = basePtr.bindMemory(to: UInt8.self).baseAddress!
                        let esc = escPtr.bindMemory(to: UInt8.self).baseAddress!
                        for r in 0..<rows {
                            let rowBase = r * groups
                            var rowMin: UInt8 = 255, rowMax: UInt8 = 0
                            for c in 0..<groups {
                                let v = src[rowBase + c]
                                if v < rowMin { rowMin = v }
                                if v > rowMax { rowMax = v }
                            }
                            let spanOK = (Int(rowMax) - Int(rowMin)) <= 15
                            var pairwiseOK = true
                            for k in 0..<(groups/2) { if src[rowBase + 2*k] != src[rowBase + 2*k+1] { pairwiseOK = false; break } }
                            if spanOK && pairwiseOK {
                                base[r]=0x00; esc[r]=0
                                for k in 0..<(groups/2) { dst[r*(groups/2)+k]=src[rowBase+2*k] }
                            } else {
                                base[r]=0xFF; esc[r]=1
                                for k in 0..<(groups/2) { dst[r*(groups/2)+k]=0 }
                            }
                        }
                    }
                }
        ...
        let packed = MLXArray(packedBytes, [rows, groups/2], dtype: .uint8)
        let base = MLXArray(baseBytes, [rows], dtype: .uint8)
        let escape = MLXArray(escapeBytes, [rows], dtype: .uint8)
        eval(packed, base, escape)
        return (packed, base, escape)
    } else if scales.dtype == .bfloat16 {
        // bfloat16 scales (group-32 affine INT8): 2 bytes per scale
        let mlxData = scales.asData(access: .noCopy)
        guard mlxData.data.count == rows * groups * 2 else { return nil }
        var packedBytes = Data(count: rows * (groups/2) * 2)
        var baseBytes = Data(count: rows)
        var escapeBytes = Data(count: rows)
        mlxData.data.withUnsafeBytes { srcPtr in
            packedBytes.withUnsafeMutableBytes { dstPtr in
                baseBytes.withUnsafeMutableBytes { basePtr in
                    escapeBytes.withUnsafeMutableBytes { escPtr in
                        let src = srcPtr.bindMemory(to: UInt16.self).baseAddress!
                        let dst = dstPtr.bindMemory(to: UInt16.self).baseAddress!
                        let base = basePtr.bindMemory(to: UInt8.self).baseAddress!
                        let esc = escPtr.bindMemory(to: UInt8.self).baseAddress!
                        for r in 0..<rows {
                            let rowBase = r * groups
                            var rowMin: UInt16 = 0xFFFF, rowMax: UInt16 = 0
                            for c in 0..<groups {
                                let v = src[rowBase + c].littleEndian
                                if v < rowMin { rowMin = v }
                                if v > rowMax { rowMax = v }
                            }
                            // Span check in bfloat domain approximated via u16 distance <=15 steps
                            let spanOK = (Int(rowMax) - Int(rowMin)) <= 15
                            var pairwiseOK = true
                            for k in 0..<(groups/2) { if src[rowBase+2*k] != src[rowBase+2*k+1] { pairwiseOK=false; break } }
                            if spanOK && pairwiseOK {
        ...
        let packed = MLXArray(packedBytes, [rows, groups/2], dtype: .bfloat16)
        let base = MLXArray(baseBytes, [rows], dtype: .uint8)
        let escape = MLXArray(escapeBytes, [rows], dtype: .uint8)
        eval(packed, base, escape)
        return (packed, base, escape)
...
// v9: packed o_proj lane-major - one nibble+base per lane vs 3 strided
private func lagunaGateSoftplusPackedSource(heads: Int) -> String {
    var src = lagunaGateSoftplusSource(heads: heads)
    src = src.replacingOccurrences(of: "const device bfloat* sc=scales+orow*KG+lane/SS;", with: "const device uint8_t* psc=(const device uint8_t*)packed_scales+orow*(KG/2)+lane/(SS*2); const device uint8_t* sbase=(const device uint8_t*)scale_base+orow; const device bfloat* sc=scales+orow*KG+lane/SS;")
    src = src.replacingOccurrences(of: "float s=float(sc[row*KG]),b=float(bs[row*KG]),a=0.0f;", with: "uint8_t sb=sbase[row]; float s=(sb==0xFF)?float(sc[row*KG]):float(psc[row*(KG/2)+lane/(SS*2)]),b=float(bs[row*KG]),a=0.0f;")
    return src
}

private let lagunaGateSoftplusPackedKernels: [Int: MLXFast.MLXFastKernel] = {
    var result: [Int: MLXFast.MLXFastKernel] = [:]
    for heads in [LagunaConstants.slidingAttentionHeads, LagunaConstants.fullAttentionHeads] {
        result[heads] = MLXFast.metalKernel(
            name: "laguna_gate_sp_h\(heads)_packed_v1",
            inputNames: ["input", "packed_codes", "scales", "biases", "packed_scales", "scale_base"],
            outputNames: ["gate_values"],
            source: lagunaGateSoftplusPackedSource(heads: heads),
            ensureRowContiguous: true)
    }
    return result
}()
...
private func lagunaGateSoftplus(
    ...
    // v9: packed lane-major path
    if let packed = bank.packedAttentionScales, let base = bank.attentionScaleBase,
       let pKernel = lagunaGateSoftplusPackedKernels[heads],
       packed.shape == [heads, LagunaConstants.hiddenSize / 64],
       base.shape == [heads] {
        return pKernel(
            [input, bank.packedCodes, bank.scales, biases, packed, base],
            grid: ((heads / 8) * 64, 1, 1),
            threadGroup: (64, 1, 1),
            outputShapes: [[1, 1, heads]],
            outputDTypes: [.bfloat16])[0]
    }
    guard let kernel = lagunaGateSoftplusKernels[heads] else { return nil }
    return kernel(
...
// v6: packed-scale variant - halves scale traffic, 0xFF sentinel -> stock (bfloat16 scales)
private func lagunaNormAffineQKVPackedSource(rows: Int, staged: Bool) -> String {
    var src = lagunaNormAffineQKVSource(rows: rows, staged: staged)
    src = src.replacingOccurrences(of: "const device bfloat* sc = weight_scales + out_row * in_vec_size_g +", with: "const device bfloat* psc = (const device bfloat*)packed_scales + out_row * (in_vec_size_g/2) +; const device uint8_t* sbase = (const device uint8_t*)scale_base + out_row; const device bfloat* sc = weight_scales + out_row * in_vec_size_g +")
    src = src.replacingOccurrences(of: "float scale = float(sc[row * in_vec_size_g]);", with: "float scale; uint8_t sb = sbase[row]; if (sb == 0xFF) scale = float(sc[row * in_vec_size_g]); else { uint pg = (in_vec_size_g/2); uint pIdx = (simd_lid / scale_step_per_thread)/2; scale = float(psc[row * pg + pIdx]); }")
    return src
}

private func lagunaNormAffineQKVPackedPrefetchSource(rows: Int, depth: Int) -> String {
    var src = lagunaNormAffineQKVPrefetchSource(rows: rows, depth: depth)
    src = src.replacingOccurrences(of: "const device bfloat* sc = weight_scales + out_row * in_vec_size_g +", with: "const device bfloat* psc = (const device bfloat*)packed_scales + out_row * (in_vec_size_g/2) +; const device uint8_t* sbase = (const device uint8_t*)scale_base + out_row; const device bfloat* sc = weight_scales + out_row * in_vec_size_g +")
    src = src.replacingOccurrences(of: "float scale = float(sc[row * in_vec_size_g]);", with: "float scale; uint8_t sb = sbase[row]; if (sb == 0xFF) scale = float(sc[row * in_vec_size_g]); else { uint pg = (in_vec_size_g/2); uint pIdx = (simd_lid / scale_step_per_thread)/2; scale = float(psc[row * pg + pIdx]); }")
    return src
}

private let lagunaNormAffineQKVPackedKernels: [Int: MLXFast.MLXFastKernel] = {
    var kernels: [Int: MLXFast.MLXFastKernel] = [:]
    let kvRows = 2 * LagunaConstants.numKeyValueHeads * LagunaConstants.headDim
    for heads in [LagunaConstants.slidingAttentionHeads, LagunaConstants.fullAttentionHeads] {
        for gateRows in [0, heads] {
            ...
            let pf = staged ? 0 : lagunaNormAffineQKVPrefetchDepth
            kernels[rows] = MLXFast.metalKernel(
                name: pf > 0 ? "laguna_norm_affine_qkv_qmv_i8g32_r\(rows)_pf\(pf)_packed_v1" : "laguna_norm_affine_qkv_qmv_i8g32_r\(rows)_packed_v1",
                inputNames: ["residual", "norm_weight", "weight_codes", "weight_scales", "weight_biases", "packed_scales", "scale_base"],
                outputNames: ["projected"],
                source: pf > 0 ? lagunaNormAffineQKVPackedPrefetchSource(rows: rows, depth: pf) : lagunaNormAffineQKVPackedSource(rows: rows, staged: staged),
                ensureRowContiguous: true
            )
        }
    }
    ...
    func callAsFunction(
            ...
            gateProjection.weight.shape == [nHeads, LagunaConstants.hiddenSize]
        {
                ...
                let fusedAffine = _nativeAffineQKV
            {
                ...
                {
                    // v5: packed-scale path for 2.588 halving - if packed
                    // available, dispatch packed kernel that halves scale
                    // traffic (stock stays resident for 0xFF escapes).
                    if let packed = fusedAffine.packedAttentionScales,
                       let base = fusedAffine.attentionScaleBase,
                       let packedKernel = lagunaNormAffineQKVPackedKernels[fusedAffine.originalShape[0]] {
                        fusedQKV = packedKernel(
                            [input, inputNorm.weight, fusedAffine.packedCodes, fusedAffine.scales, affineBiases, packed, base],
                            grid: ((fusedAffine.originalShape[0] / 8) * 64, 1, 1),
                            threadGroup: (64, 1, 1),
                            outputShapes: [[1, 1, fusedAffine.originalShape[0]]],
                            outputDTypes: [.bfloat16]
                        )[0]
                    } else {
