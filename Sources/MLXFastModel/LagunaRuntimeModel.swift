    ...
    if doSort {
        ...
        if storesEligible || loadsEligible {
            ...
            if loadsSelected {
                ...
                // instead). The VALUES are nonetheless the TRUE source rows
                // (`order.floorDivide(expertsPerTok)`, all < numTokens), not
                // the encoded keys: if any frozen or fallback path ever does
                // consume lhs_indices as a gather index, benign in-range
                // values degrade to a correct physical gather instead of an
                // out-of-bounds device access. Post-mortem hardening after
                // ranked no-scores c2e292c7/835b22f7 isolated the failure to
                // this lhs route.
                let expertsPerTok = indices.dim(-1)
                sortedX = expandedX.flattened(start: 0, end: -3)
                gateUpLHSIndices = order.floorDivide(expertsPerTok).asType(.uint32)
            } else {
                let expertsPerTok = indices.dim(-1)
