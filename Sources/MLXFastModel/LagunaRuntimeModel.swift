...
/// Kernel ledger instrument -- see `research/PREFILL_LEDGER_INSTRUMENT.md`
/// and `research/artifacts/README-kernel-ledger.md`.
///
/// This is a deliberate, openly-documented *slowdown*, not an optimization.
/// The ranked M5 selects `_nax` gather-GEMM kernels that no host available to
/// this campaign can execute, so no local profile can price them. Recomputing
/// one kernel family `m` extra times over the same supplied tokens and reading
/// the resulting shift in the receipt's raw `prefill_seconds_per_token` /
/// `decode_seconds_per_token` yields that family's wall time on the official
/// hardware directly.
///
/// Each repetition is attached with `MLX.depends`, whose `eval_gpu` only calls
/// `copy_shared_buffer` and dispatches no kernel. The replica's value is never
/// read, so every emitted token is unchanged structurally rather than by an
/// IEEE-754 identity, and the injected slope carries no attachment overhead.
///
/// A shipped optimization candidate sets all four to zero. The defaults below
/// are receipt dose R-1 (`m_p = 1`, `m_d = 6`); see
/// `research/frieren-pr148-preregistration.md` section 4.

/// Routed gate/up gather-GEMM, multi-token sorted prefill path only.
let lagunaLedgerRoutedGateUpReps = max(
    0,
    ProcessInfo.processInfo.environment["DARKBLOOM_LEDGER_GATEUP_REPS"]
        .flatMap(Int.init) ?? 1
)
/// Routed down-projection gather-GEMM, multi-token sorted prefill path only.
let lagunaLedgerRoutedDownReps = max(
    0,
    ProcessInfo.processInfo.environment["DARKBLOOM_LEDGER_DOWN_REPS"]
        .flatMap(Int.init) ?? 0
)
/// Routed gate/up QMV + SwiGLU, single-token decode path only.
let lagunaLedgerDecodeGateUpReps = max(
    0,
    ProcessInfo.processInfo.environment["DARKBLOOM_LEDGER_DECODE_GATEUP_REPS"]
        .flatMap(Int.init) ?? 6
)
/// Control family: `k` extra `lagunaLedgerFold(activated, activated)` on both
/// the prefill and the decode routed activation. `(a + a) * 0.5 == a` exactly,
/// so this injects only the fold's own dispatches and traffic and no kernel
/// work at all. On prefill (`activated` is 4 MiB/layer) it prices the fold
/// contamination that must be subtracted from the real families' slopes; on
/// decode (`activated` is 8 KiB/layer) the traffic is negligible and the slope
/// is a direct per-dispatch price on the ranked hardware.
let lagunaLedgerNullFoldReps = max(
    0,
    ProcessInfo.processInfo.environment["DARKBLOOM_LEDGER_NULL_FOLD_REPS"]
        .flatMap(Int.init) ?? 0
)
/// Cache-residency control for the decode gate/up replicas. When set, the
/// replicas are issued through the explicit-indices packed entry point with
/// each routed expert index rotated by this many slots (mod 256). `0` keeps
/// the shipped expert set, so the replica re-reads the lines the shipped
/// dispatch just touched; `128` selects a disjoint set of eight experts, so
/// the replica must fetch its 9.44 MB gate/up slab from DRAM. The pair
/// isolates last-level-cache warmth from the marginal byte rate. Leaving the
/// variable unset keeps the replica byte-identical to the shipped dispatch.
let lagunaLedgerDecodeRotate: Int? =
    ProcessInfo.processInfo.environment["DARKBLOOM_LEDGER_DECODE_ROTATE"]
        .flatMap(Int.init)
...
/// Ledger-instrument fold (see `lagunaLedgerRoutedGateUpReps`). `a` and `b` are
/// two independent evaluations of the same deterministic expression, so they
/// are bitwise equal; `(a + b) * 0.5` then reproduces `a` exactly in IEEE-754,
/// including signed zero, for every finite input whose doubling does not
/// overflow. Routed activations here are O(1e2) against a BF16 ceiling of
/// 3.39e38, so the overflow branch is unreachable. Kept as one function so the
/// fold's own elementwise cost -- which must be subtracted from a measured
/// family time -- has a single definition.
private func lagunaLedgerFold(_ a: MLXArray, _ b: MLXArray) -> MLXArray {
    (a + b) * 0.5
}
...
private func lagunaFusedSortedRoutedGateUp(
    ...
    func gateUpActivation() -> MLXArray {
        ...
        if lagunaExpertAlignedGatherEnabled {
            ...
            // restoring the logical shape expected by down_proj. Both steps are
            // zero-copy views of a contiguous prefix, so on the ranked
            // expert-aligned path this closure is exactly one gather-QMM
            // dispatch and the ledger prices that family alone.
        ...
        return lagunaInterleavedSwiGLU(gateUp, split: split)
    }
    var activated = gateUpActivation()
    if lagunaLedgerRoutedGateUpReps > 0 {
        activated = MLX.depends(
            input: activated,
            dependencies: (0 ..< lagunaLedgerRoutedGateUpReps).map { _ in gateUpActivation() })
    }
    for _ in 0 ..< lagunaLedgerNullFoldReps {
        activated = lagunaLedgerFold(activated, activated)
    }
    // SwitchGLU: `x = downProj(activated, idx, sortedIndices: doSort)`
    var result = downProj(activated, idx, sortedIndices: doSort)
    if lagunaLedgerRoutedDownReps > 0 {
        result = MLX.depends(
            input: result,
            dependencies: (0 ..< lagunaLedgerRoutedDownReps).map { _ in
                downProj(activated, idx, sortedIndices: doSort)
            })
    }
...
final class LagunaRuntimeSparseMoEBlock: Module, UnaryLayer {
    ...
    private func forward(
            ...
            x.dim(1) == 1, inds.size < 64
        {
            ...
            func decodeGateUpActivation() -> MLXArray {
                let activated: MLXArray
            ...
            var activated = decodeGateUpActivation()
            if lagunaLedgerDecodeGateUpReps > 0 {
                var replica = decodeGateUpActivation
                if let rotate = lagunaLedgerDecodeRotate,
                    ...
                    inds.dims(1, 1, LagunaConstants.numExpertsPerTok)
                {
                    // Both rotations emit the same two elementwise ops, so the
                    // rotation arithmetic cancels out of the arm-to-arm contrast.
                    let rotated = MLX.bitwiseAnd(
                        inds + UInt32(rotate), UInt32(LagunaConstants.numExperts - 1))
                    replica = {
                        lagunaRoutedSwiGLUQMVPacked(
                            x,
                            fusedWeight: fusedWeight,
                            packedScales: packedBank,
                            indices: rotated
                ...
                activated = MLX.depends(
                    input: activated,
                    dependencies: (0 ..< lagunaLedgerDecodeGateUpReps).map { _ in replica() })
            }
            for _ in 0..<lagunaLedgerNullFoldReps {
                activated = lagunaLedgerFold(activated, activated)
            }
