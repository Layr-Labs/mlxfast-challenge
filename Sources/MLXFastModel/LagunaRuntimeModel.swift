...
/// Official-seed path: pack the final K/V cache layout in the 39 ordinary
/// nonterminal layers. The specialized terminal-prefill layer stays stock.
...
private func lagunaSeedFinalKVReceipt(_ result: MLXArray, cache: [KVCache]) {
    ...
    for entry in cache {
        ...
        if let full = entry as? KVCacheSimple, full.seedFinalBackingAdopted {
            seedFinalAdopted += 1
        }
        if let ring = entry as? RotatingKVCache, ring.seedFinalBackingAdopted {
            seedFinalAdopted += 1
        }
        for array in state {
    ...
    let eligibleCount = cache.count - 1
    let line = "seed_final_kv_receipt offset=\(offset) lockstep_offsets=40/40 "
        + "seed_final_adopted=\(seedFinalAdopted)/\(eligibleCount) "
        + "terminal_seed_path=stock digest=\(String(digest, radix: 16))\n"
...
final class LagunaRuntimeAttention: Module {
    ...
    func callAsFunction(
        ...
        if lagunaSeedFinalKVEnabled && L == 512 {
            precondition(
