...
/// inside the atlas, the angle row is an atlas SLICE (a view, no kernel at
/// all) instead of running the family's RoPE over the probe seed. Row
/// `position` of the atlas is byte-identical to the scalar-offset probe at
/// that position (`prepareRoPEAngleAtlases` builds it with the very same
/// RoPE instances), so every consumer sees the same floats. This is not the
/// embedding-fusion atlas path measured at −0.23% above — that experiment
/// replaced the probes with a fused KERNEL whose fixed cost exceeded them;
/// a view has no kernel cost. DEFAULT ON; set
/// `DARKBLOOM_ROPE_ANGLE_SLICE=0` to restore the per-step probe dispatches.
...
private func lagunaGateQMVProductSoftplusSource(heads: Int) -> String {
    ...
    for (uint elem = lid; elem < uint(in_vec); elem += 512) {
...
private let lagunaGateQMVProductSoftplusKernels: [Int: MLXFast.MLXFastKernel] = {
    ...
    for heads in [LagunaConstants.slidingAttentionHeads, LagunaConstants.fullAttentionHeads] {
        kernels[heads] = MLXFast.metalKernel(
            name: "laguna_gate_qmv_product_softplus_bf16_h\(heads)_v1",
            inputNames: [
...
final class LagunaRuntimeModelInner: Module {
    ...
    private func ropeAngleTable(
        ...
        if lagunaRoPEAngleSliceEnabled, let atlas,
            position >= 0, position < lagunaRoPEAngleAtlasLength
        {
            return atlas[.ellipsis, position ..< (position + 1), 0...]
        }
