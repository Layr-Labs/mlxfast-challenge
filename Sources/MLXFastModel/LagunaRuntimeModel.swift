...
func lagunaGatedAffineOProjNVFP4Source(
    ...
    uint column = simd_lid * values_per_thread;
    // Depth-1 staging (K1/K3-receipted class): block k+1's four rows' code
    // pairs and scale bytes issue before block k's dot products consume k's
    // registers. Same bytes/addresses/decode/order.
    uint2 st_cw[results_per_simdgroup];
    uint8_t st_sb[results_per_simdgroup];
    #pragma unroll
    for (uint row = 0; row < results_per_simdgroup; ++row) {
        st_cw[row] = *(const device uint2*)(ws + row * (in_vec_size / 8));
        st_sb[row] = sc[row * in_vec_size_g];
    }
    for (uint k = 0; k < in_vec_size; k += block_size) {
        \(loadInput)

        uint2 cur_cw[results_per_simdgroup];
        uint8_t cur_sb[results_per_simdgroup];
        #pragma unroll
        for (uint row = 0; row < results_per_simdgroup; ++row) {
            cur_cw[row] = st_cw[row];
            cur_sb[row] = st_sb[row];
        }
        if (k + block_size < in_vec_size) {
            const device uint32_t* ws_next = ws + block_size / 8;
            const device uint8_t* sc_next = sc + block_size / group_size;
            #pragma unroll
            for (uint row = 0; row < results_per_simdgroup; ++row) {
                st_cw[row] = *(const device uint2*)(
                    ws_next + row * (in_vec_size / 8));
                st_sb[row] = sc_next[row * in_vec_size_g];
            }
        }
        ...
        for (uint row = 0; row < results_per_simdgroup; ++row) {
            ...
            uint8_t sbits = cur_sb[row];
            \(scaleDecode)
            \(accumDecl)
            // Same 8B-aligned code pair, staged: identical words in
            // identical j order.
            const uint2 cw = cur_cw[row];
            #pragma unroll
