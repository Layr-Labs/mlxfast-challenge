...
/// `DARKBLOOM_SHARED_GATE_UP_ANY_L` (default on; set "0" to disable):
/// prefill/multi-token counterpart to `DARKBLOOM_FUSED_SHARED_GATE_UP` above.
/// Serves the shared expert's gate/up projection for any sequence length L
/// (not just single-token decode) from the same retained row-concatenated
/// NVFP4 `[gate; up]` bank, via one `MLX.quantizedMM` dispatch instead of two
/// separate `Linear` calls. `MLX.quantizedMM` is already shape-generic --
/// it is exactly what `QuantizedLinear.callAsFunction` calls for any input
/// rank (see `Quantized.swift`) -- and each output row's K-loop and scale
/// application reads only its own weight/scale row, independent of sequence
/// position or gate/up bank membership, so the fused dispatch is bit-exact
/// against the two separate `gateProj`/`upProj` calls it replaces for any L.
/// Same argument the decode-only branch below already relies on, extended
/// from L=1 to any L. Measured ~-2.6% prefill standalone (note `ba4561c`,
/// never landed as its own submission -- the bundled submission `61ffecc`
/// that shipped it alongside an unrelated LUT change failed pre-scoring for
/// reasons unrelated to this lever's own correctness/performance profile).
/// Ships default ON, independently ablatable from the decode-only flag.
let lagunaSharedGateUpAnyLEnabled =
    ProcessInfo.processInfo.environment["DARKBLOOM_SHARED_GATE_UP_ANY_L"] != "0"
...
final class LagunaRuntimeMLP: Module, UnaryLayer {
    ...
    func callAsFunction(_ x: MLXArray) -> MLXArray {
        ...
        if lagunaSharedGateUpAnyLEnabled,
            let fusedWeight = _fusedGateUpWeight, let fusedScales = _fusedGateUpScales,
            x.dtype == .bfloat16,
            x.dim(0) == 1,
            ...
            _fusedGateUpSplit == LagunaConstants.sharedExpertIntermediateSize
        {
            // Prefill/multi-token counterpart of the decode-only fused bank
            // QMM above. `MLX.quantizedMM` is shape-generic -- it is
            // literally what `QuantizedLinear.callAsFunction` dispatches for
            // any input rank -- so this is the exact same row-concatenation
            // exactness argument as the L==1 branch above, extended to any L.
            lagunaTrace("shared fused [gate; up] bank QMM (any L)")
            let gateUp = MLX.quantizedMM(
                x,
                fusedWeight,
                scales: fusedScales,
