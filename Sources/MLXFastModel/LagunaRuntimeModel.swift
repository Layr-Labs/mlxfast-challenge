...
/// Official-seed dose: pack the final K/V cache layout in the 10 ordinary
/// full-attention layers. Every sliding layer, including terminal, stays stock.
...
private func lagunaSeedFinalKVReceipt(_ result: MLXArray, cache: [KVCache]) {
    ...
    for entry in cache {
        ...
        if let full = entry as? KVCacheSimple {
            fullLayerCount += 1
            if full.seedFinalBackingAdopted { seedFinalAdopted += 1 }
        }
        if let ring = entry as? RotatingKVCache {
            slidingLayerCount += 1
            precondition(!ring.seedFinalBackingAdopted)
        }
        for array in state {
    ...
    let line = "seed_final_kv_receipt offset=\(offset) lockstep_offsets=40/40 "
        + "full_seed_final_adopted=\(seedFinalAdopted)/\(fullLayerCount) "
        + "sliding_seed_path=stock terminal_seed_path=stock "
