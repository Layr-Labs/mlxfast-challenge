...
/// (`DARKBLOOM_OPROJ_GEOM=g<simdgroups>x<rows>`, default `g4x4` — the winner
/// of the mirrored decode A/B; `g2x4` restores the stock shape) so QKV and
/// o_proj geometry experiments never confound each other. Same bit-exactness
/// argument as the QKV knob: whole-row granularity only.
private let lagunaOProjNVFP4Geometry: (numSimdgroups: Int, rowsPerSimdgroup: Int) = {
    let raw = ProcessInfo.processInfo.environment["DARKBLOOM_OPROJ_GEOM"] ?? "g4x4"
...
/// (default `g8x4`: 256-thread threadgroups of eight simdgroups at four rows
/// each — measured -1.8% decode wall vs the stock `g2x4` 64-thread geometry
/// in mirrored 400-step A/B; `g2x4` restores the stock `fp_qmv_fast` shape).
...
private let lagunaTailNormQKVGateGeometry: (numSimdgroups: Int, rowsPerSimdgroup: Int) = {
    let raw = ProcessInfo.processInfo.environment["DARKBLOOM_TAIL_QKV_GEOM"] ?? "g8x4"
    let parts = raw.dropFirst().split(separator: "x")
    guard raw.hasPrefix("g"), parts.count == 2,
        let ns = Int(parts[0]), let rps = Int(parts[1]),
        [1, 2, 4, 8].contains(ns), rps > 0, rps <= 16
