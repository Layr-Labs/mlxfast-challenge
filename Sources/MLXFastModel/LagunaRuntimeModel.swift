...
private let lagunaSharedSwiGLUQMVRows1Kernel = MLXFast.metalKernel(
    ...
    source: """
        ...
        for (uint block = 0; block < input_width; block += block_width) {
            if (block == 0) {
                // Consume the prefetched vectors: identical values into
                // identical slots in identical order.
                input_values[0] = pf0[0];
                input_values[1] = pf0[1];
                input_values[2] = pf0[2];
                input_values[3] = pf0[3];
                input_values[4] = pf1[0];
                input_values[5] = pf1[1];
                input_values[6] = pf1[2];
                input_values[7] = pf1[3];
                input_values[8] = pf2[0];
                input_values[9] = pf2[1];
                input_values[10] = pf2[2];
                input_values[11] = pf2[3];
                input_values[12] = pf3[0];
                input_values[13] = pf3[1];
                input_values[14] = pf3[2];
                input_values[15] = pf3[3];
            } else {
                const device vec<bfloat, 4>* input_vectors =
                    (const device vec<bfloat, 4>*) (
                        input + block + lane * values_per_lane);
...
private let lagunaRoutedSwiGLUQMVPackedTop8R1Kernel = MLXFast.metalKernel(
    ...
    source: """
        ...
        uint lane = thread_index_in_simdgroup;
        uint logical_row = tile * 2 + simd_group;
        // Depth-1 input prefetch (the receipted QKV-prefetch template): the
        // input stream is addressed by lane alone, so block 0's 32B issue
        // ABOVE the serial top-8 extraction chain that gates every weight
        // address. Same bytes, consumed into the same slots in the same
        // order; only load issue time moves.
        const device vec<bfloat, 4>* pf_vectors =
            (const device vec<bfloat, 4>*)(input + lane * values_per_lane);
        vec<bfloat, 4> pf0 = pf_vectors[0];
        vec<bfloat, 4> pf1 = pf_vectors[1];
        vec<bfloat, 4> pf2 = pf_vectors[2];
        vec<bfloat, 4> pf3 = pf_vectors[3];
