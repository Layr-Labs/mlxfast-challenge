...
let lagunaSlidingPipeDepth: Int = {
    guard let raw = ProcessInfo.processInfo.environment["DARKBLOOM_SLIDING_PIPE_DEPTH"],
...
private func lagunaSlidingFusedAttentionKLoopSource(depth: Int) -> String {
    let letters = ["a", "b", "c", "d", "e", "f", "g", "h"]
    var lines: [String] = []
    lines.append("int i = sg;")
    let guardExpr = depth == 1 ? "i < N" : "i + \(depth - 1) * BN < N"
    lines.append("for (; \(guardExpr); i += \(depth) * BN) {")
    for s in 1..<depth {
        let stride = s == 1 ? "inner_k_stride" : "\(s) * inner_k_stride"
        lines.append(
            "    const device bfloat* pipe_keys_\(letters[s]) = pair_keys + \(stride);")
    }
    for s in 1..<depth {
        let stride = s == 1 ? "inner_v_stride" : "\(s) * inner_v_stride"
        lines.append(
            "    const device bfloat* pipe_values_\(letters[s]) = pair_values + \(stride);")
    }
    for s in 0..<depth {
        let slot = s == 0 ? "i" : (s == 1 ? "i + BN" : "i + \(s) * BN")
        lines.append("    const bool sub_\(letters[s]) = uint(\(slot)) == widx;")
    }
    for s in 0..<depth {
        lines.append("    U pipe_k\(letters[s])[4];")
    }
    for s in 0..<depth {
        let ptr = s == 0 ? "pair_keys" : "pipe_keys_\(letters[s])"
        lines.append("    T_LOAD_K(pipe_k\(letters[s]), sub_\(letters[s]), \(ptr));")
    }
    for s in 0..<depth {
        let l = letters[s]
        lines.append("    bfloat pipe_v\(l)0, pipe_v\(l)1, pipe_v\(l)2, pipe_v\(l)3;")
    }
    for s in 0..<depth {
        let l = letters[s]
        let ptr = s == 0 ? "pair_values" : "pipe_values_\(l)"
        lines.append("    T_LOAD_V(pipe_v\(l)0, pipe_v\(l)1, pipe_v\(l)2, pipe_v\(l)3, sub_\(l),")
        lines.append("        \(ptr));")
    }
    for s in 0..<depth {
        let l = letters[s]
        let p = s == 0 ? "pair" : "pipe\(l)"
        lines.append("")
        lines.append("    U \(p)_score0 = 0;")
        lines.append("    U \(p)_score1 = 0;")
        for j in 0..<4 {
            lines.append("    \(p)_score0 += pair_q0[\(j)] * pipe_k\(l)[\(j)];")
            lines.append("    \(p)_score1 += pair_q1[\(j)] * pipe_k\(l)[\(j)];")
        }
        lines.append("    \(p)_score0 = simd_sum(\(p)_score0);")
        lines.append("    \(p)_score1 = simd_sum(\(p)_score1);")
        lines.append("")
        lines.append("    U \(p)_new_max0 = metal::max(pair_max0, \(p)_score0);")
        lines.append("    U \(p)_new_max1 = metal::max(pair_max1, \(p)_score1);")
        lines.append("    U \(p)_factor0;")
        lines.append("    U \(p)_factor1;")
        lines.append("    LAGUNA_RESCALE(\(p)_factor0, pair_max0 - \(p)_new_max0);")
        lines.append("    LAGUNA_RESCALE(\(p)_factor1, pair_max1 - \(p)_new_max1);")
        lines.append("    U \(p)_exp0 = metal::fast::exp(\(p)_score0 - \(p)_new_max0);")
        lines.append("    U \(p)_exp1 = metal::fast::exp(\(p)_score1 - \(p)_new_max1);")
        lines.append("")
        lines.append("    pair_max0 = \(p)_new_max0;")
        lines.append("    pair_max1 = \(p)_new_max1;")
        lines.append("    pair_sum0 = pair_sum0 * \(p)_factor0 + \(p)_exp0;")
        lines.append("    pair_sum1 = pair_sum1 * \(p)_factor1 + \(p)_exp1;")
        lines.append("")
        for j in 0..<4 {
            lines.append(
                "    pair_o0[\(j)] = pair_o0[\(j)] * \(p)_factor0 + \(p)_exp0 * pipe_v\(l)\(j);")
            lines.append(
                "    pair_o1[\(j)] = pair_o1[\(j)] * \(p)_factor1 + \(p)_exp1 * pipe_v\(l)\(j);")
        }
    }
    lines.append("")
    lines.append("    pair_keys += \(depth) * inner_k_stride;")
    lines.append("    pair_values += \(depth) * inner_v_stride;")
    lines.append("}")
    return lines.joined(separator: "\n")
}

private func lagunaSlidingFusedAttentionKernelForDepth(depth: Int)
    -> MLXFast.MLXFastKernel
{
    MLXFast.metalKernel(
    name: "laguna_sliding_fused_attn_ring_v1_pd\(depth)",
    inputNames: [
...
\(lagunaSlidingFusedAttentionKLoopSource(depth: depth))
...
private let lagunaSlidingFusedAttentionKernel =
    lagunaSlidingFusedAttentionKernelForDepth(depth: lagunaSlidingPipeDepth)
...
// r104a-ladder-leg-02-armA-depth4
