...
private let lagunaSlidingFusedAttentionPC1Kernel = MLXFast.metalKernel(
    ...
    inputNames: [
        "raw_queries", "raw_keys", "raw_values",
        "query_weight", "key_weight", "angles",
        "k_cache", "v_cache", "scale_arr",
    ],
    outputNames: ["attended"],
    source: lagunaSlidingFusedAttentionSource.replacingOccurrences(
...
private let lagunaFullFusedAttentionPC1Kernel = MLXFast.metalKernel(
    ...
    outputNames: ["attended"],
    source: lagunaFullFusedAttentionSource.replacingOccurrences(
        of: """
        uint widx = params[0];
        int N = int(params[1]);
        uint capacity = params[2];
...
/// The ordinary 512-token warm covers all PC0 identities but its first full
/// decode grows the cache, so PC1 needs one explicit kernel-only warm.
func lagunaWarmAttentionParamsRoPESidecarKernels(_ model: LagunaRuntimeModel) {
    ...
    let fh = LagunaConstants.fullAttentionHeads
    let full = lagunaFullFusedAttention(
        rawQueries: MLXArray.zeros([1, 1, fh * d], dtype: .bfloat16),
    ...
    let sh = LagunaConstants.slidingAttentionHeads
    let window = LagunaConstants.slidingWindow
    let sliding = lagunaSlidingFusedAttention(
        rawQueries: MLXArray.zeros([1, 1, sh * d], dtype: .bfloat16),
        rawKeys: keys, rawValues: values, queryWeight: qw, keyWeight: kw,
        ...
        writeIdx: 1, scale: scale, sidecarProof: proof)
    lagunaTrace("warm attention params rope pc1 identities")
    eval([produced.hidden, produced.fullAngles, produced.slidingAngles, full, sliding])
}
