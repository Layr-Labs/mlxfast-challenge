...
/// Default-on PC1 metadata carrier for architectural full-attention layers.
/// Sliding layers keep their ordinary PC0 parameter binding and angle width.
let lagunaAttentionParamsRoPESidecarEnabled =
    ProcessInfo.processInfo.environment[
        "DARKBLOOM_ATTN_PARAMS_ROPE_SIDECAR"] != "0"
/// Full-attention PC2 defaults on after PC1 proves the sidecar descriptor.
/// Set `DARKBLOOM_ATTN_SCALE_LITERAL=0` to retain PC1's scalar scale input.
...
func lagunaSlidingFusedAttention(
    ...
    lagunaTrace("sliding fused attention pc0")
    ...
    return lagunaSlidingFusedAttentionPC0Kernel(
        commonInputs + [params, scale],
...
/// Ordinary warm covers full PC0; warm full PC1/PC2 and sliding PC0 explicitly.
func lagunaWarmAttentionParamsRoPESidecarKernels(_ model: LagunaRuntimeModel) {
    ...
    let proof = LagunaDecodeAttentionParamsProof(
        position: 0, fullWriteIdx: 1, fullCapacity: 2)
    ...
    func slidingPC0() -> MLXArray {
        lagunaSlidingFusedAttention(
            ...
            writeIdx: 1, scale: scale)
    }
    let fullPC1 = full(nil)
    var warmOutputs = [
        produced.hidden, produced.fullAngles, produced.slidingAngles,
        fullPC1, slidingPC0(),
    ]
    if lagunaAttentionScaleLiteralEnabled {
        warmOutputs.append(full(lagunaAttentionScaleBits))
    }
    lagunaTrace("warm full attention pc1/pc2, sliding pc0")
    eval(warmOutputs)
}
...
private let lagunaDecodeEmbeddingRoPEAtlasSidecarKernel = MLXFast.metalKernel(
    name: "laguna_decode_embedding_rope_atlas_bf16_2048_full_pc1_v1",
    inputNames: ["tokens", "embedding_weight", "full_atlas", "sliding_atlas",
        "decode_params"],
    outputNames: ["hidden", "full_angles", "sliding_angles"],
...
private func lagunaDecodeEmbeddingRoPEAtlas(
    ...
    lagunaTrace(useSidecar
        ? "decode embedding+rope atlas full pc1 sidecar"
...
final class LagunaRuntimeModelInner: Module {
    ...
    /// Read-only full-family certificate; each full consumer rechecks at prepare.
    private func decodeAttentionParamsProof(
        ...
        for (index, layerType) in layerTypes.enumerated() {
            guard layerType == .full else { continue }
        ...
        guard let fullDescriptor else { return nil }
    ...
    func callAsFunction(_ inputs: MLXArray, cache: [KVCache]? = nil) -> MLXArray {
        ...
        for (i, layer) in layers.enumerated() {
            let isSliding = layerTypes[i] == .sliding
            let isFull = !isSliding
            let mask = isFull ? fullMask : slidingMask
            let qkRoPEAngles = isFull ? fullRoPEAngles : slidingRoPEAngles
            let layerAttentionParamsProof =
                !isSliding ? attentionParamsProof : nil
            if i == layers.count - 1, h.dim(1) > 1 {
                if case .causal = mask {
                    ...
                    h = layer(
                        ...
                        qkRoPEAngles: qkRoPEAngles,
                        qkRoPEOffsets: qkRoPEOffsets,
                        attentionParamsProof: layerAttentionParamsProof
                    )
