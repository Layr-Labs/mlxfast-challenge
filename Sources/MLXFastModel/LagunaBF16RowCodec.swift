...
private func lagunaBF16RowCodecMetalHeader(layout: LagunaBF16RowCodecLayout) -> String {
    """
...
private let lagunaSlidingOCodecLayout = LagunaBF16RowCodecLayout(
    columns: LagunaConstants.slidingAttentionHeads * LagunaConstants.headDim)

private func lagunaLosslessSlidingOProjectionSource(unroll: Int) -> String {
    """
    constexpr uint unroll = \(unroll);
    constexpr uint in_vec_size = \(lagunaSlidingOCodecLayout.columns);
    constexpr uint heads = \(LagunaConstants.slidingAttentionHeads);
...
private let lagunaLosslessSlidingOProjectionKernels: [Int: MLXFast.MLXFastKernel] = {
    var result: [Int: MLXFast.MLXFastKernel] = [:]
    for unroll in [1, 2, 4, 8] {
        result[unroll] = MLXFast.metalKernel(
            name: "laguna_lossless_sliding_o_bf16_u\(unroll)_v1",
            inputNames: [
                "attention_output", "gate_values", "raw_weight",
                "packed_weight", "row_tiers", "row_extra_offsets",
            ],
            outputNames: ["projected"],
            source: lagunaLosslessSlidingOProjectionSource(unroll: unroll),
            header: lagunaBF16RowCodecMetalHeader(layout: lagunaSlidingOCodecLayout),
            ensureRowContiguous: true)
    }
    return result
}()

func lagunaLosslessSlidingOProjection(
    ...
    guard let kernel = lagunaLosslessSlidingOProjectionKernels[unroll] else { return nil }
    let inVec = lagunaSlidingOCodecLayout.columns
    ...
    precondition(gateValues.shape == [1, 1, LagunaConstants.slidingAttentionHeads])
    precondition(rawWeight.dtype == .bfloat16)
    precondition(rawWeight.shape == [LagunaConstants.hiddenSize, inVec])
    precondition(codec.layout == lagunaSlidingOCodecLayout)
    ...
    lagunaTrace("lossless sliding O projection u\(unroll)")
    return kernel(
        [
...
private let lagunaBF16RowCodecReconstructKernel = MLXFast.metalKernel(
    name: "laguna_bf16_row_codec_reconstruct_bits_v1",
    inputNames: [
        "raw_words", "packed_weight", "row_tiers", "row_extra_offsets",
    ],
    outputNames: ["reconstructed"],
    source: """
        constexpr uint columns = \(lagunaSlidingOCodecLayout.columns);
...
func lagunaBF16RowCodecReconstructBits(
    ...
    precondition(codec.layout == lagunaSlidingOCodecLayout)
    return lagunaBF16RowCodecReconstructKernel(
