...
public final class LagunaRuntimeWeightCache {
    ...
    /// The ticket is retained forever -- ending it would `resize(0)` the
    /// residency set and unwire the weights.
    ///
    /// SHIPPED DOSE (submission chunk 1): capacity = 0.001 x live bytes
    /// + 8 MiB ~= 42 MiB. The measured dose curve on the M5 Max (cool-gated
    /// Latin squares, medians, correctness green on every sample):
    ...
    /// at ~5%, so this chunk ships the 42 MiB dose and later submissions
    /// raise the default toward full wire in band-sized steps.
    ///
    ...
    private static let wiredZHDefaultFraction = 0.001
    private static let wiredZHDefaultSlackMB = 8
